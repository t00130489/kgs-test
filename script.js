// PCでEnterキーで早押し・次の問題へボタンを押せるように
document.addEventListener('keydown', function(e) {
  if (e.key === 'Enter' && !e.repeat) {
    const t = e.target;
    // 入力系要素内での Enter はグローバル処理しない（回答送信やフォーム用途専用）
    if (t && (t.tagName === 'INPUT' || t.tagName === 'TEXTAREA' || t.isContentEditable)) return;
    // 早押しボタン優先
    if (!buzzBtn.disabled && buzzBtn.offsetParent !== null) {
      buzzBtn.click();
      e.preventDefault();
      return;
    }
    // 次の問題へボタン
    if (!nextBtn.disabled && nextBtn.offsetParent !== null) {
      nextBtn.click();
      e.preventDefault();
      return;
    }
  }
});
// script.js

// セクション：定数
const TEXT = {
  preCountdownSec: 3,
  questionTimeLimit: 30,
  answerTimeLimit: 15,
  typeSpeed: 100,
  labels: {
    timeoutLabel: '残り ',
    secondsSuffix: '秒',
    statusAllWrong: '全員誤答… 正解： ',
    statusTimeUp: '時間切れ！ 正解： ',
    statusCorrect: '正解！🎉',
    statusWrong: guess => `不正解（${guess}）`,
    nextQuestion: '次の問題へ',
    finalResult: '最終結果へ',
    returnHome: 'トップページに戻る',
    resultsTitle: '結果',
    participantsHeader: 'メンバーと正解数',
    perQuestionHeader: '問題別 回答一覧',
    correctLabel: '正解者： ',
    incorrectLabel: '不正解者： ',
    timeoutLabelList: 'タイムアウト',
    leaveConfirm: 'ゲームから離脱します。よろしいですか？',
    homeConfirm: 'ホーム画面に戻ります。\nこのゲームには自動で復帰しません。よろしいですか？',
    questionLabelPrefix: '第',
    questionLabelSuffix: '問'
  }
};

// 章の表示名（ログ画面と共通の並び）
const CHAPTER_NAMES = ['序章','第一章','第二章','第三章','第四章','第五章','第六章','第七章'];
// 「大学院の範囲」に含まれる章
const GRAD_CHAPTERS = [0, 1, 4, 7];

// 離脱確認制御
// ゲーム進行中（クイズ画面が表示されている）ときだけ確認する。
// ホーム画面や結果画面では出さない。
let allowUnload = false;
function isInGame(){
  try {
    if (allowUnload) return false;
    if (!roomId || !myNick) return false;
    return !!quizAppDiv && !quizAppDiv.classList.contains('hidden');
  } catch(_) { return false; }
}
window.addEventListener('beforeunload', e => {
  if (!isInGame()) return;
  e.preventDefault();
  e.returnValue = TEXT.labels.leaveConfirm;
});
// Service Worker の更新をゲーム中は保留させるためのフック（index.html から参照）
window.__kgsIsInGame = isInGame;

// Firebase 初期化
import { initializeApp } from "https://www.gstatic.com/firebasejs/12.0.0/firebase-app.js";
import {
  getDatabase, ref, set, push,
  onValue, onChildAdded, remove,
  get, child, update,
  runTransaction, onDisconnect
} from "https://www.gstatic.com/firebasejs/12.0.0/firebase-database.js";

initializeApp({
  apiKey: "AIzaSyBYWW8Ldgtow1fxctSqIZynLpxFwRAcc-c",
  authDomain: "kgs-test-68924.firebaseapp.com",
  databaseURL: "https://kgs-test-68924-default-rtdb.firebaseio.com",
  projectId: "kgs-test-68924",
  storageBucket: "kgs-test-68924.appspot.com",
  messagingSenderId: "806988019711",
  appId: "1:806988019711:web:3859c3fa8182371761d9ca",
  measurementId: "G-QEP0467K9D"
});
const db = getDatabase();
const FUNCTIONS_BASE = 'https://us-central1-kgs-test-68924.cloudfunctions.net';

// サーバ時刻オフセット取得
// 初回同期が返るまでは端末のローカル時刻のままなので、
// ルーム作成/参加はこれを待ってから行う（端末の時計ずれがそのまま入るのを防ぐ）。
let serverTimeOffset = 0;
let serverTimeSynced = false;
let resolveServerTime;
const serverTimeReady = new Promise(res => { resolveServerTime = res; });
onValue(ref(db, '.info/serverTimeOffset'), s => {
  serverTimeOffset = s.val() || 0;
  serverTimeSynced = true;
  if (resolveServerTime) { resolveServerTime(); resolveServerTime = null; }
});
const getServerTime = () => Date.now() + serverTimeOffset;
async function waitServerTime(timeoutMs = 3000){
  if (serverTimeSynced) return;
  await Promise.race([serverTimeReady, new Promise(r => setTimeout(r, timeoutMs))]);
}

// DOM取得

const roomCountInput = document.getElementById('room-count');
const createBtn   = document.getElementById('createBtn');
const joinRoomBtn = document.getElementById('joinRoomBtn');
const roomIdInput = document.getElementById('roomIdInput');
joinRoomBtn.disabled = true;
// モード選択スイッチ取得（未チェック=入力, チェック=選択）
const modeSwitch = document.getElementById('mode-switch');
if (modeSwitch) {
  // 既定は入力（未チェック）。ARIAの状態同期
  modeSwitch.checked = false;
  modeSwitch.setAttribute('aria-checked', String(modeSwitch.checked));
  modeSwitch.addEventListener('change', () => {
    modeSwitch.setAttribute('aria-checked', String(modeSwitch.checked));
  });
}


// バリデーション用エラー表示

function showInputError(input, message) {
  input.setCustomValidity(message);
  input.reportValidity();
}

// 出題問題数バリデーション

roomCountInput.addEventListener('input', () => {
  const val = roomCountInput.value;
  const n = Number(val);
  if (!val.match(/^[0-9]{1,3}$/) || !Number.isFinite(n) || n < 1 || n > 999) {
    showInputError(roomCountInput, '1～999の数字を入力してください');
    createBtn.disabled = true;
  roomCountInput.classList.remove('valid-input');
  } else {
    showInputError(roomCountInput, '');
    createBtn.disabled = false;
  roomCountInput.classList.add('valid-input');
  }
});
roomIdInput.addEventListener('input', () => {
  const val = roomIdInput.value;
  if (val.match(/^[0-9]{4}$/)) {
    showInputError(roomIdInput, '');
    joinRoomBtn.disabled = false;
    roomIdInput.classList.add('valid-input');
  } else {
    showInputError(roomIdInput, '4桁の数字のみ入力してください');
    joinRoomBtn.disabled = true;
    roomIdInput.classList.remove('valid-input');
  }
});
// Enter キーで参加ボタンを押下可能に
roomIdInput.addEventListener('keydown', e => {
  if (e.key === 'Enter' && !joinRoomBtn.disabled) {
    e.preventDefault();
    joinRoomBtn.click();
  }
});

// --- 軽量トースト ---
const toastContainer = document.createElement('div');
toastContainer.className = 'toast-container';
document.body.appendChild(toastContainer);
function showToast(message, actionLabel, onAction, timeout=3000) {
  const el = document.createElement('div');
  el.className = 'toast';
  const msg = document.createElement('div');
  msg.className = 'toast-msg';
  msg.textContent = message;
  el.appendChild(msg);
  let timer;
  if (actionLabel && onAction) {
    const act = document.createElement('div');
    act.className = 'toast-action';
    act.textContent = actionLabel;
    act.addEventListener('click', () => { try { onAction(); } catch(_){}; close(); });
    el.appendChild(act);
  }
  function close(){
    clearTimeout(timer);
    if (el.parentNode) el.parentNode.removeChild(el);
  }
  toastContainer.appendChild(el);
  if (timeout > 0) timer = setTimeout(close, timeout);
  return close;
}

// --- オフライン表示 ---
// Firebase に繋がっていないと画面は出るのにボタンが効かず「固まった」ように見えるため、
// 切断が続いたときだけ画面上部にバーを出す（一瞬の瞬断ではちらつかせない）。
let offlineBarEl = null;
let offlineBarTimer = null;
function setOfflineBar(show){
  if (show) {
    if (offlineBarEl) return;
    offlineBarEl = document.createElement('div');
    offlineBarEl.id = 'offline-bar';
    offlineBarEl.className = 'offline-bar';
    offlineBarEl.textContent = '接続が切れています。再接続を試みています…';
    document.body.appendChild(offlineBarEl);
  } else if (offlineBarEl) {
    if (offlineBarEl.parentNode) offlineBarEl.parentNode.removeChild(offlineBarEl);
    offlineBarEl = null;
  }
}
onValue(ref(db, '.info/connected'), snap => {
  const connected = !!snap.val();
  clearTimeout(offlineBarTimer);
  if (connected) setOfflineBar(false);
  else offlineBarTimer = setTimeout(() => setOfflineBar(true), 3000);
});

// --- 端末ID（本人性の担保） ---
// ニックネームだけでは「戻ってきた本人」と「同名の別人」を区別できないため、
// 端末に固定のIDを持たせ、players ノードに添えて保存する。
const CLIENT_ID_KEY = 'kgs.clientId';
const NICK_KEY = 'kgs.lastNick';
const clientId = (() => {
  const gen = () => {
    try {
      if (typeof crypto !== 'undefined' && crypto.randomUUID) return crypto.randomUUID();
    } catch(_) {}
    return 'c-' + Date.now().toString(36) + '-' + Math.random().toString(36).slice(2, 10);
  };
  try {
    let v = localStorage.getItem(CLIENT_ID_KEY);
    if (!v) { v = gen(); localStorage.setItem(CLIENT_ID_KEY, v); }
    return v;
  } catch(_) { return gen(); }
})();

// --- セッション永続化（再参加用） ---
const SESSION_KEY = 'kgs.session';
// この時間内なら確認なしで自動復帰する
const AUTO_REJOIN_MS = 5 * 60 * 1000;
function saveSession() {
  try {
    if (!roomId || !myNick) return;
    localStorage.setItem(SESSION_KEY, JSON.stringify({
      roomId, myNick, clientId, joinedAt: joinTs, savedAt: Date.now()
    }));
  } catch(_) {}
}
function touchSession() {
  try {
    const s = loadSession();
    if (!s || s.roomId !== roomId || s.myNick !== myNick) return saveSession();
    s.savedAt = Date.now();
    localStorage.setItem(SESSION_KEY, JSON.stringify(s));
  } catch(_) {}
}
function clearSession() {
  try { localStorage.removeItem(SESSION_KEY); } catch(_) {}
}
function loadSession() {
  try { const s = localStorage.getItem(SESSION_KEY); return s ? JSON.parse(s) : null; } catch(_) { return null; }
}

// --- 復帰導線（消えないカード） ---
function hideRejoinCard(){
  const c = document.getElementById('rejoin-card');
  if (c && c.parentNode) c.parentNode.removeChild(c);
}
function showRejoinCard(s, note){
  hideRejoinCard();
  const card = document.createElement('div');
  card.id = 'rejoin-card';
  card.className = 'card rejoin-card';

  const title = document.createElement('div');
  title.className = 'rejoin-title';
  title.textContent = `ルーム ${s.roomId} が進行中です`;

  const desc = document.createElement('div');
  desc.className = 'rejoin-desc';
  desc.textContent = note || `${s.myNick} として復帰できます`;

  const actions = document.createElement('div');
  actions.className = 'rejoin-actions';
  const ok = document.createElement('button');
  ok.className = 'btn-primary';
  ok.textContent = 'ゲームに復帰';
  ok.addEventListener('click', () => { rejoinWithSession(s); });
  const no = document.createElement('button');
  no.className = 'btn-secondary';
  no.textContent = '破棄';
  no.addEventListener('click', () => { clearSession(); hideRejoinCard(); });
  actions.appendChild(ok); actions.appendChild(no);

  card.appendChild(title); card.appendChild(desc); card.appendChild(actions);
  if (homeDiv) homeDiv.insertBefore(card, homeDiv.firstChild);
}

// ページロード時：本人だと確認できれば自動復帰、できなければ常設カードを出す
async function tryAutoRejoin() {
  const s = loadSession();
  if (!s || !s.roomId || !s.myNick) return false;
  // すでに入室済みなら何もしない
  if (roomId && myNick) return true;

  const snap = await get(child(ref(db,'rooms'), s.roomId)).catch(()=>null);
  if (!snap || !snap.exists()) { clearSession(); return false; }

  const [playersSnap, settingsSnap] = await Promise.all([
    get(ref(db,`rooms/${s.roomId}/players`)).catch(()=>null),
    get(ref(db,`rooms/${s.roomId}/settings`)).catch(()=>null)
  ]);
  const playersObj = (playersSnap && playersSnap.val()) || {};
  const settingsObj = (settingsSnap && settingsSnap.val()) || {};
  if (settingsObj.finishedAt) { clearSession(); return false; }

  const mine = playersObj[s.myNick];
  // 同名が別端末に取られている場合だけは復帰させない
  if (mine && mine.clientId && mine.clientId !== clientId) {
    showRejoinCard(s, `「${s.myNick}」は別の端末が使用中です。別の名前で参加してください。`);
    return true;
  }

  const nodeIsMine = !!(mine && (!mine.clientId || mine.clientId === clientId));
  const nodeFresh  = !!(mine && (getServerTime() - (mine.lastActive || 0)) < AUTO_REJOIN_MS);
  const sessionFresh = (Date.now() - (s.savedAt || 0)) < AUTO_REJOIN_MS;

  // 本人のノードが生きている / 直前まで参加していた → 確認なしで自動復帰
  if ((nodeIsMine && nodeFresh) || (!mine && sessionFresh)) {
    await rejoinWithSession(s, { silent: false });
    return true;
  }
  showRejoinCard(s);
  return true;
}

async function rejoinWithSession(s, opts){
  opts = opts || {};
  hideRejoinCard();
  showLoading('ゲームに復帰中...');
  try {
    const [pSnap, setSnap] = await Promise.all([
      get(ref(db,`rooms/${s.roomId}/players/${s.myNick}`)),
      get(ref(db,`rooms/${s.roomId}/settings`))
    ]);
    const settings = setSnap.val() || {};
    if (settings.finishedAt) {
      clearSession();
      showToast('このゲームは既に終了しています', null, null, 3000);
      return false;
    }
    const mine = pSnap.val();
    if (mine && mine.clientId && mine.clientId !== clientId) {
      showToast(`「${s.myNick}」は別の端末が使用中です`, null, null, 3500);
      showRejoinCard(s, `「${s.myNick}」は別の端末が使用中です。別の名前で参加してください。`);
      return false;
    }

    roomId = s.roomId; myNick = s.myNick;
    const now = getServerTime();
    const playerRef = ref(db,`rooms/${roomId}/players/${myNick}`);
    if (mine && typeof mine.joinedAt === 'number') {
      // joinedAt は不変（RTDBルール）。復帰では書き換えない。
      joinTs = mine.joinedAt;
      await update(playerRef, { lastActive: now, connected: true, clientId });
    } else {
      // ノードが消えている → 元の参加時刻を復元して作り直す
      joinTs = (typeof s.joinedAt === 'number' && s.joinedAt) ? s.joinedAt : now;
      await set(playerRef, { joinedAt: joinTs, lastActive: now, connected: true, clientId });
    }
    // イベント再生の起点はローカル値として別管理（joinedAt とは切り離す）
    eventCutoffTs = now;
    settingsCache = settings;
    roomModeValue = settings.mode || 'input';

    saveSession();
    homeDiv.classList.add('hidden');
    quizAppDiv.classList.remove('hidden');
    currentRoom.textContent = roomId;
    applyModeUI();

    attachPresence();
    startHeartbeat();
    attachWatchers();
    await resyncGameState('rejoin');
    if (!opts.silent) showToast('ゲームに復帰しました', null, null, 2000);
    return true;
  } catch(e) {
    console.error('rejoin failed', e);
    showToast('復帰に失敗しました。もう一度お試しください。', null, null, 3000);
    return false;
  } finally { hideLoading(); }
}
window.addEventListener('DOMContentLoaded', () => {
  roomIdInput.classList.remove('valid-input');
  roomCountInput.classList.remove('valid-input');
});
const homeDiv     = document.getElementById('home');
const quizAppDiv  = document.getElementById('quiz-app');
const resultsDiv  = document.getElementById('results');
const currentRoom = document.getElementById('currentRoomId');
const chapterCbs  = document.querySelectorAll('#chapter-selection input[type=checkbox][value]');
const gradCb      = document.getElementById('grad-range');
const allCb       = document.getElementById('all-range');
const roomCount   = document.getElementById('room-count');
const roomRange   = document.getElementById('room-range');
const currentNum  = document.getElementById('currentNum');
const totalNum    = document.getElementById('totalNum');
const roomMode    = document.getElementById('room-mode');
const playersUl   = document.getElementById('players');
const statusEl    = document.getElementById('status');
const preCd       = document.getElementById('pre-countdown');
const questionEl  = document.getElementById('question');
const qTimerEl    = document.getElementById('question-timer');
const buzzBtn     = document.getElementById('buzzBtn');
const answerArea  = document.getElementById('answer-area');
const answerInput = document.getElementById('answerInput');
const answerBtn   = document.getElementById('answerBtn');
const aTimerEl    = document.getElementById('answer-timer');
const nextBtn     = document.getElementById('nextBtn');
const homeBtn     = document.getElementById('homeBtn');
const startBtn    = document.getElementById('startBtn');
const choiceArea  = document.getElementById('choice-area');

// ホスト用キャプション
const hostCaption = document.createElement('div');
hostCaption.id = 'host-caption';
hostCaption.textContent = '参加者が揃ったら問題スタート！（一人でもOK！）';
hostCaption.style = 'color:#222; font-size:0.95rem; margin-top:1.2rem; margin-bottom:1.2rem; text-align:center; display:block;';

// ホスト用キャプションとボタンを縦並び中央揃えでラップするdiv
const hostWrap = document.createElement('div');
hostWrap.id = 'host-caption-wrap';
hostWrap.style = 'display:flex; flex-direction:column; align-items:center; width:100%;';

// 参加者用キャプション
const waitCaption = document.createElement('div');
waitCaption.id = 'wait-caption';
waitCaption.textContent = '主催者が問題をスタートするのを待っています...';
waitCaption.style = 'color:#222; font-size:0.95rem; margin-bottom:12rem; text-align:center;';

// ニックネーム入力モーダル生成
const nicknameModal = document.createElement('div');
nicknameModal.id = 'nickname-modal';
nicknameModal.style = `
  position: fixed; left: 0; top: 0; width: 100vw; height: 100vh; z-index: 2000;
  background: rgba(0,0,0,0.35); display: none; align-items: center; justify-content: center;
  overflow: auto;
`;
nicknameModal.innerHTML = `
  <div style="background: #fff; border-radius: 12px; padding: 2rem 1.5rem; min-width: 280px; box-shadow: 0 4px 24px rgba(0,0,0,0.18); text-align: center;">
    <div style="font-size: 1.1rem; margin-bottom: 1rem;">あなたのニックネームを入力してください</div>
    <input id="nickname-input" type="text" maxlength="15" style="width: 90%; font-size: 1.1rem; padding: 0.5rem; border-radius: 6px; border: 1px solid #ccc; margin-bottom: 1.2rem;" autocomplete="off">
    <br>
  <button id="nickname-ok" class="btn-primary" style="font-size: 1.1rem; padding: 0.6rem 1.5rem; margin-right: 1.2rem;" disabled>OK</button>
    <button id="nickname-cancel" class="btn-secondary" style="font-size: 1.1rem; padding: 0.6rem 1.5rem; background: #e0e0e0; color: #555; border: 1px solid #ccc;">キャンセル</button>
  </div>
`;
document.body.appendChild(nicknameModal);

function showNicknameModal() {
  return new Promise(resolve => {
    const modal = nicknameModal;
    const input = modal.querySelector('#nickname-input');
    const okBtn = modal.querySelector('#nickname-ok');
    const cancelBtn = modal.querySelector('#nickname-cancel');
    modal.style.display = 'flex';
    // 前回使った名前を初期値に入れる（毎回打ち直さずに済むように）
    let remembered = '';
    try { remembered = localStorage.getItem(NICK_KEY) || ''; } catch(_) {}
    input.value = remembered;
    okBtn.disabled = !remembered;
    // iOS対策：ユーザー操作の文脈で一度不可視inputへフォーカスしてキーボードを開き、すぐに実入力へ移譲
    (function focusNicknameField(){
      try {
        if (typeof isIOS === 'function' && isIOS()) {
          // 既存プロキシがあれば破棄
          if (window.focusProxyInput) {
            try { window.focusProxyInput.blur(); } catch(_) {}
            if (window.focusProxyInput.parentNode) window.focusProxyInput.parentNode.removeChild(window.focusProxyInput);
            window.focusProxyInput = null;
          }
          const proxy = document.createElement('input');
          proxy.type = 'text';
          proxy.autocapitalize = 'none';
          proxy.autocomplete = 'off';
          proxy.spellcheck = false;
          proxy.style.position = 'fixed';
          proxy.style.opacity = '0';
          proxy.style.pointerEvents = 'none';
          proxy.style.zIndex = '-1';
          proxy.style.left = '0';
          proxy.style.bottom = '0';
          proxy.style.width = '1px';
          proxy.style.height = '1px';
          document.body.appendChild(proxy);
          proxy.focus();
          window.focusProxyInput = proxy;
          // すぐに本来の入力へフォーカス移譲
          input.focus();
          try { if (input.value) input.select(); else input.setSelectionRange(0, 0); } catch(_) {}
          // 保険として少し遅延しても再フォーカス
          setTimeout(() => { try { input.focus(); } catch(_) {} }, 0);
        } else {
          // 非iOSは通常フォーカス
          input.focus();
          try { if (input.value) input.select(); } catch(_) {}
        }
      } catch(_) {}
      if (window.innerWidth <= 600) {
        setTimeout(() => {
          try { input.scrollIntoView({behavior:'smooth', block:'center'}); } catch(_) {}
        }, 150);
      }
    })();
    function close(val) {
      modal.style.display = 'none';
      okBtn.removeEventListener('click', onOk);
      cancelBtn.removeEventListener('click', onCancel);
      input.removeEventListener('keydown', onKey);
      input.removeEventListener('input', onInput);
      // iOS用のフォーカスプロキシを破棄
      if (window.focusProxyInput) {
        try { window.focusProxyInput.blur(); } catch(_) {}
        if (window.focusProxyInput.parentNode) window.focusProxyInput.parentNode.removeChild(window.focusProxyInput);
        window.focusProxyInput = null;
      }
      resolve(val);
    }
    function onOk() {
      const val = input.value.trim();
      if (val) {
        try { localStorage.setItem(NICK_KEY, val); } catch(_) {}
        close(val);
      } else input.focus();
    }
    function onCancel() {
      close(null);
    }
    function onKey(e) {
      if(e.key==='Enter') onOk();
      if(e.key==='Escape') onCancel();
    }
    function onInput() {
      const has = input.value.trim().length > 0;
      okBtn.disabled = !has;
    }
    okBtn.addEventListener('click', onOk);
    cancelBtn.addEventListener('click', onCancel);
    input.addEventListener('keydown', onKey);
    input.addEventListener('input', onInput);
  });
}

// 「第〇問」ラベル
const questionLabelEl = document.getElementById('question-label');
const questionCardBlock = document.getElementById('question-card-block');
// questionElは既に上部で宣言済みなので再宣言しない

// フィードバックオーバーレイ取得
const feedbackOverlay = document.getElementById('feedback-overlay');
// Confetti canvas
const confettiCanvas = document.getElementById('confetti-canvas');
let confettiCtx = confettiCanvas ? confettiCanvas.getContext('2d') : null;

/**
 * 描画バッファ(width/height属性)をCSSボックスの実寸に合わせる。
 * canvasはバッファとCSSサイズが別管理で、ズレるとズレた軸だけ非等方に
 * 引き伸ばされる（スマホではアドレスバーの出入りやキーボード開閉で
 * innerWidthは変わらず高さだけ変わるため、縦に伸びたまま固定されていた）。
 * リサイズのたびに合わせるとバッファがクリアされるので、バースト開始時にだけ測る。
 * @returns {{w:number,h:number}} CSSピクセルでの描画領域サイズ
 */
function syncConfettiCanvas() {
  const rect = confettiCanvas.getBoundingClientRect();
  const dpr = Math.min(window.devicePixelRatio || 1, 2);
  const w = Math.max(1, Math.round(rect.width * dpr));
  const h = Math.max(1, Math.round(rect.height * dpr));
  if (confettiCanvas.width !== w || confettiCanvas.height !== h) {
    confettiCanvas.width = w;
    confettiCanvas.height = h;
  }
  // 以降はCSSピクセル座標で描ける（バッファ再確保で変換が消えるため毎回設定）
  confettiCtx.setTransform(dpr, 0, 0, dpr, 0, 0);
  return { w: rect.width, h: rect.height };
}

// Sound disabled by request
function playJingle() { /* disabled */ }

// Confetti particle system (very small, single burst)
// 座標・速度・サイズはすべてCSSピクセル基準。物理量は画面の短辺に比例させ、
// スマホとPCで見え方の縮尺が揃うようにする。
function burstConfetti({x=null, y=null, count=90}={}) {
  if (!confettiCtx || !confettiCanvas) return;
  if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  const { w, h } = syncConfettiCanvas();
  const originX = (x == null) ? w / 2 : x;
  const originY = (y == null) ? h * 0.35 : y;
  const scale = Math.min(w, h) / 800;          // 速度・重力は画面比例
  const sizeScale = Math.max(0.75, Math.min(1.4, scale)); // 紙片は小さくなりすぎない程度に
  const colors = ['#ff5252','#ffb300','#ffd54f','#4dd0e1','#81c784','#ba68c8'];
  const parts = Array.from({length: Math.max(20, Math.min(200, count))}, () => ({
    x: originX, y: originY,
    vx: (Math.random()*2-1) * 6 * scale,
    vy: ((Math.random()*-1) * (6+Math.random()*6) - 4) * scale,
    size: (3 + Math.random()*4) * sizeScale,
    rot: Math.random()*Math.PI*2,
    vr: (Math.random()*2-1) * 0.2,
    color: colors[(Math.random()*colors.length)|0],
    life: 700 + Math.random()*500
  }));
  const start = performance.now();
  let last = start;
  const grav = 0.18 * scale;
  function frame(t) {
    const dt = Math.min(t - last, 50); last = t;
    const k = dt / (1000 / 60); // 60fps基準の進行係数（120Hz端末で倍速にならないように）
    confettiCtx.clearRect(0, 0, w, h);
    let alive = false;
    for (const p of parts) {
      p.life -= dt;
      if (p.life <= 0) continue;
      alive = true;
      p.vy += grav * k;
      p.x += p.vx * k;
      p.y += p.vy * k;
      p.rot += p.vr * k;
      confettiCtx.save();
      confettiCtx.translate(p.x, p.y);
      confettiCtx.rotate(p.rot);
      confettiCtx.fillStyle = p.color;
      confettiCtx.fillRect(-p.size/2, -p.size/2, p.size, p.size);
      confettiCtx.restore();
    }
    if (alive && t - start < 1600) requestAnimationFrame(frame);
    else confettiCtx.clearRect(0, 0, w, h);
  }
  requestAnimationFrame(ts => { last = ts; frame(ts); });
}

// Floating score bubble
function floatScore(text='+1') {
  const el = document.createElement('div');
  el.className = 'float-score';
  // split into styled spans for richer look
  el.innerHTML = `<span class="fs-text">${text}</span><span class="fs-spark">✨</span>`;
  document.body.appendChild(el);
  setTimeout(() => { if (el.parentNode) el.parentNode.removeChild(el); }, 1100);
}

function celebrateCorrect(options={}) {
  try {
    // subtle card glow
    const card = document.getElementById('question-card-block');
    if (card) {
      card.classList.remove('celebrate-glow');
      // force reflow to restart animation
      void card.offsetWidth;
      card.classList.add('celebrate-glow');
    }
    burstConfetti({ count: options.count || 100 });
    playJingle(options.pitch || 880);
    floatScore(options.scoreText || '+1');
  } catch(_) {}
}

// 簡易ローディングオーバーレイ
let loadingOverlay;
function showLoading(text){
  if (!loadingOverlay) {
    loadingOverlay = document.createElement('div');
    loadingOverlay.id = 'loading-overlay';
    loadingOverlay.style = 'position:fixed;inset:0;background:rgba(0,0,0,0.35);display:flex;align-items:center;justify-content:center;z-index:2500;color:#fff;font-size:1.2rem;';
    const box = document.createElement('div');
    box.style = 'background:rgba(0,0,0,0.7);padding:1rem 1.5rem;border-radius:10px;min-width:200px;text-align:center;box-shadow:0 4px 18px rgba(0,0,0,0.25)';
    const spinner = document.createElement('div');
    spinner.style = 'width:28px;height:28px;border:3px solid #fff;border-top-color:transparent;border-radius:50%;margin:0 auto 0.6rem;animation:spin 0.8s linear infinite;';
    const label = document.createElement('div');
    label.id = 'loading-label';
    label.textContent = text || '読み込み中...';
    box.appendChild(spinner); box.appendChild(label);
    loadingOverlay.appendChild(box);
    document.body.appendChild(loadingOverlay);
    const style = document.createElement('style');
    style.textContent = '@keyframes spin{from{transform:rotate(0)}to{transform:rotate(360deg)}}';
    document.head.appendChild(style);
  } else {
    const label = loadingOverlay.querySelector('#loading-label');
    if (label) label.textContent = text || '読み込み中...';
    loadingOverlay.style.display = 'flex';
  }
}
function hideLoading(){ if (loadingOverlay) loadingOverlay.style.display = 'none'; }

// 観戦バナー（カード内）
function showSpectatorBanner() {
  const wrap = document.querySelector('#question-card-block .question-card-center');
  if (!wrap) return;
  let banner = document.getElementById('spectator-banner');
  if (!banner) {
    banner = document.createElement('div');
    banner.id = 'spectator-banner';
    banner.textContent = '観戦中 — 次の問題から参加できます';
    banner.style = 'margin:0.4rem auto; color:#555; font-weight:bold; background:#f5f7fa; border:1px dashed #bbb; border-radius:8px; padding:0.5rem 0.8rem;';
    wrap.appendChild(banner);
  }
}
function hideSpectatorBanner() {
  const b = document.getElementById('spectator-banner');
  if (b && b.parentNode) b.parentNode.removeChild(b);
}

// フィードバック表示（アニメーションをスムーズに）
function showFeedback(isCorrect) {
  feedbackOverlay.textContent = isCorrect ? '〇' : '×';
  feedbackOverlay.classList.remove('hidden', 'correct', 'wrong', 'animating');
  // 強制再描画でアニメーションリセット
  void feedbackOverlay.offsetWidth;
  feedbackOverlay.classList.add(isCorrect ? 'correct' : 'wrong', 'animating');
  feedbackOverlay.classList.remove('hidden');
  // 触覚フィードバック
  if (window.navigator.vibrate) {
    window.navigator.vibrate(isCorrect ? [30, 30, 30] : [80, 40, 80]);
  }
  setTimeout(() => {
    feedbackOverlay.classList.remove('animating');
    feedbackOverlay.classList.add('hidden');
  }, 700);
}

// ハプティクスのみ発火（オーバーレイは表示しない）
function triggerHaptic(isCorrect) {
  try {
    if (window.navigator && window.navigator.vibrate) {
      window.navigator.vibrate(isCorrect ? [30, 30, 30] : [80, 40, 80]);
    }
  } catch(_) {}
}

// 状態変数
let quizData = [], sequence = [], idx = 0;
// 解決済み（正解/時間切れ/全員誤答）になった問題のインデックス
let resolvedIndex = null;
let myNick = '', roomId = '', joinTs = 0;
let players = {}, scores = {}, wrongs = {};
let settingsCache = {};
// タイマー表示最適化用（直前表示した残り秒：0.1秒単位の文字列）
let lastDisplayedQTenth = null;
let lastDisplayedQSec = null;
let flowStarted = false, answered = false;
let questionStart = 0, remainingQTime = TEXT.questionTimeLimit;
let nextBtnCountdownTimer = null;
let nextBtnCountdownRemain = 0;
let roomModeValue = 'input';
let handledCorrectFor = new Set();
// 勝者の紙吹雪/+1 を重複表示しないためのフラグ
let didLocalWinCelebrate = false;
// 自分がこの問題で誤答（または解答時間切れ）したかどうかのローカルフラグ
// DBの反映前でも自分だけは早押しボタンを再有効化しないために使用
let iAmWrongLocal = false;
// 追加: 早押し中断での残り時間とタイプ同期解除関数
let pausedRemainingQTime = null; // 秒
let heartbeatTimer = null; // 接続維持用ハートビート
let unsubs = []; // onValue等の解除関数を保持
// --- 離脱/復帰まわり ---
const PRESENCE_GRACE_MS = 120 * 1000;               // 一時離席を在室扱いする猶予
const STALE_BUZZ_MS = (TEXT.answerTimeLimit + 5) * 1000; // 放置された早押しを解放するまで
let presenceUnsub = null;       // .info/connected の監視解除
let myBuzzOnDisconnect = null;  // 自分が確保した buzz の onDisconnect
let currentBuzz = null;         // 現在の buzz スナップショット
let lastBuzzRemain = null;      // buzz 時点の残り時間（全端末で共有し再開時刻を揃える）
let staleBuzzTimer = null;      // 放置 buzz の解放タイマー
let eventCutoffTs = 0;          // これ以前の events は再生しない（joinedAt とは別管理）
let resyncPending = false;      // resyncGameState の多重実行防止
let lastHiddenAt = 0;           // 直近でバックグラウンドに入った時刻
// 途中参加者向け観戦モード（現在進行中の問題はスキップし、次の問題から同期）
let spectatorUntilNext = false;
let preStartSkipTs = null; // スキップ対象の preStart タイムスタンプ
// 現在の問題の preStart タイムスタンプ（全員誤答判定で観戦者を除外するために使用）
let currentPreStartTs = null;
// sequence件数が要求より少ない場合のトーストを一度だけ出すためのフラグ
let notifiedReducedCount = false;
// iOS用：ユーザー操作のコンテキストでキーボードを開いておくためのフォーカスプロキシ
let focusProxyInput = null;
function isIOS(){
  try {
    const ua = navigator.userAgent || '';
    const isIdevice = /iPhone|iPad|iPod/i.test(ua);
    const isMacTouch = navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1;
    return isIdevice || isMacTouch;
  } catch(_) { return false; }
}

// タイマー＆タイプクリア
function clearTimers(){
  clearInterval(window._preInt);
  clearTimeout(window._preTo);
  clearInterval(window._qInt);
  clearInterval(window._aInt);
  stopTypewriter();
  clearNextBtnCountdown();
}
function clearNextBtnCountdown() {
  if (nextBtnCountdownTimer) {
    clearInterval(nextBtnCountdownTimer);
    nextBtnCountdownTimer = null;
    nextBtnCountdownRemain = 0;
    // ボタンのラベルを元に戻す
    if (nextBtn) {
      const isFinal = idx+1>=sequence.length;
      nextBtn.textContent = isFinal ? TEXT.labels.finalResult : TEXT.labels.nextQuestion;
      nextBtn.style.minWidth = '';
      // ボタン色リセット
      if (isFinal) {
        nextBtn.classList.add('btn-danger');
        nextBtn.classList.remove('btn-primary');
      } else {
        nextBtn.classList.remove('btn-danger');
        nextBtn.classList.add('btn-primary');
      }
    }
  }
}

// タイプを即時に全文表示し、タイプ同期も解除（余計な追記防止）
function revealFullQuestionAndStopSync() {
  stopTypewriter();
  currentText = sequence[idx] && sequence[idx].question ? sequence[idx].question : (currentText || '');
  if (typeof currentText !== 'string') currentText = '';
  questionEl.textContent = currentText;
  typePos = currentText.length;
}
function canBuzz(){ return flowStarted && !answered && !wrongs[myNick] && !iAmWrongLocal; }
function updateBuzzState(){
  const disabled = spectatorUntilNext ? true : !canBuzz();
  buzzBtn.disabled = disabled;
  buzzBtn.classList.toggle('disabled-btn', disabled);
}

// --- 待機中UI（作成/参加/復帰で共通） ---
function applyModeUI(){
  if (roomMode) roomMode.textContent = roomModeValue === 'select' ? '選択' : '入力';
  if (roomModeValue === 'select') {
    buzzBtn.style.display = 'none';
    choiceArea.classList.add('hidden');
  } else {
    buzzBtn.style.display = '';
    choiceArea.classList.add('hidden');
  }
}
function showHostStartUI(){
  if (!startBtn) return;
  if (document.getElementById('wait-caption')) { try { waitCaption.remove(); } catch(_) {} }
  let wrap = document.getElementById('host-caption-wrap');
  if (!wrap) {
    wrap = document.createElement('div');
    wrap.id = 'host-caption-wrap';
    wrap.style = 'display:flex; flex-direction:column; align-items:center; width:100%';
    const row = startBtn.parentNode || document.querySelector('.start-row');
    if (row) row.appendChild(wrap);
  }
  if (!wrap.contains(hostCaption)) wrap.appendChild(hostCaption);
  if (!wrap.contains(startBtn)) wrap.appendChild(startBtn);
  startBtn.style.display = 'block';
}
function showWaitCaption(){
  if (startBtn) startBtn.style.display = 'none';
  if (document.getElementById('host-caption')) { try { hostCaption.remove(); } catch(_) {} }
  const w = document.getElementById('host-caption-wrap');
  if (w && !w.contains(startBtn)) w.remove();
  if (!document.getElementById('wait-caption') && buzzBtn && buzzBtn.parentNode) {
    buzzBtn.parentNode.insertBefore(waitCaption, buzzBtn);
  }
}

// --- 選択モードの選択肢 ---
function lockChoicesShowingAnswer(){
  if (!choiceArea) return;
  Array.from(choiceArea.children).forEach(b => {
    if (b.dataset.isAnswer === '1') b.classList.add('btn-danger');
    else b.classList.add('disabled-btn');
    b.disabled = true;
  });
}
// 選択中の章に何問あるかを表示する（作ってみるまで分からないのを避ける）
let chapterCounts = null;
async function loadChapterCounts(){
  if (chapterCounts) return chapterCounts;
  try {
    // 問題バンク全件(182KB)を落とさず、章ごとの件数だけを取る
    const resp = await fetch(`${FUNCTIONS_BASE}/getQuestionCounts`);
    if (!resp.ok) return null;
    const data = await resp.json();
    chapterCounts = (data && data.counts) || null;
    return chapterCounts;
  } catch(_) { return null; }
}
function renderRangeTotal(){
  const el = document.getElementById('range-total');
  if (!el) return;
  const chs = [...chapterCbs].filter(cb=>cb.checked).map(cb=>+cb.value);
  if (!chs.length) { el.textContent = ''; return; }
  if (!chapterCounts) { el.textContent = ''; return; }
  const total = chs.reduce((s,c) => s + (chapterCounts[c] || 0), 0);
  el.textContent = `問題総数：${total}問`;
}
function updateCreateBtn(){
  const hasChapters = [...chapterCbs].some(cb=>cb.checked);
  const val = roomCountInput.value;
  const n = Number(val);
  const validCount = (/^[0-9]{1,3}$/.test(val) && Number.isFinite(n) && n>=1 && n<=999);
  createBtn.disabled = !(hasChapters && validCount);
  renderRangeTotal();
}

// 質問タイマー（0.1秒単位表示）
function tickQ(){
  if(!flowStarted) return;
  const now = getServerTime();
  const elapsedSec = (now - questionStart) / 1000;
  const remain = Math.max(0, TEXT.questionTimeLimit - elapsedSec);
  remainingQTime = remain; // float 秒保持
  // 0.1秒単位で切り上げ表示（0.0 になるまで 0.1 単位で減少）
  const remainTenth = Math.ceil(remain * 10) / 10;
  const disp = remainTenth.toFixed(1);
  if (lastDisplayedQTenth !== disp) {
    lastDisplayedQTenth = disp;
    qTimerEl.textContent = TEXT.labels.timeoutLabel + disp + TEXT.labels.secondsSuffix;
  }
  if(remain <= 0 && !answered){
    clearTimers();
    onQuestionTimeout();
  }
}

// タイムアップ：問題制限時間
function onQuestionTimeout(){
  clearTimers();
  answered = true;
  flowStarted = false;
  // 解決済みとしてマーキング（next の二重進行防止用）
  resolvedIndex = idx;
  revealFullQuestionAndStopSync();
  qTimerEl.textContent = '正解：' + sequence[idx].answer;
  qTimerEl.classList.add('show-answer');
  qTimerEl.style.display = 'block';
  statusEl.textContent = '時間切れ！';
  buzzBtn.disabled = true;
  exitAnswerMode();
  aTimerEl.style.display = 'none';
  nextBtn.disabled = false;
  startNextBtnCountdown();
  releaseBuzzOnDisconnect();
  clearStaleBuzzWatch();
  currentBuzz = null;
  remove(ref(db, `rooms/${roomId}/buzz`));
  pausedRemainingQTime = null;
  lastBuzzRemain = null;
        if (roomModeValue === 'select') lockChoicesShowingAnswer();
}

// 初期状態
createBtn.disabled = true;
answerArea.classList.add('hidden');
answerBtn.disabled = true;
if (typeof roomModeValue === 'undefined') roomModeValue = 'input';
if (roomModeValue === 'select') {
  buzzBtn.style.display = 'none';
  choiceArea.classList.add('hidden');
} else {
  buzzBtn.disabled = true; buzzBtn.classList.add('disabled-btn');
  buzzBtn.style.display = '';
  choiceArea.classList.add('hidden');
}
startBtn.style.display = 'none';
qTimerEl.style.display = 'none';
aTimerEl.style.display = 'none';

// 範囲プリセット
gradCb.addEventListener('change', ()=>{ chapterCbs.forEach(cb=>{ if(['0','1','4','7'].includes(cb.value)) cb.checked=gradCb.checked; }); updateCreateBtn(); });
allCb.addEventListener('change', ()=>{ chapterCbs.forEach(cb=>cb.checked=allCb.checked); updateCreateBtn(); });
chapterCbs.forEach(cb=>cb.addEventListener('change',updateCreateBtn));

// 事前の全件取得は行わず、作成時にCloud Functionsから取得する
updateCreateBtn();
loadChapterCounts().then(renderRangeTotal);

// ウォッチャー
function renderPlayers(){
  playersUl.innerHTML='';
  const names = Object.keys(players);
  names.forEach(nick=>{
    const p = players[nick];
    const li = document.createElement('li');
    const name = document.createElement('span');
    name.textContent = `${nick} (${(scores && scores[nick])||0}問正解)`;
    li.appendChild(name);
    if (p && p.connected === false) {
      const away = isActive(p);
      li.classList.add(away ? 'player-away' : 'player-offline');
      const tag = document.createElement('span');
      tag.className = 'player-state-tag';
      tag.textContent = away ? '離席中' : 'オフライン';
      li.appendChild(tag);
    }
    playersUl.appendChild(li);
  });
}
function watchPlayers(){
  const off = onValue(ref(db,`rooms/${roomId}/players`), snap=>{
    players = snap.val()||{};
    renderPlayers();
    // 自分のエントリが消えていたら（古いクライアント等による削除）自動で復元する。
    // ただしルームが消えている/終了している場合は復元しない（ゾンビ化防止）。
    if (myNick && roomId && !players[myNick] && !resyncPending) {
      const targetRoom = roomId, targetNick = myNick;
      get(ref(db, `rooms/${targetRoom}/settings`)).then(sn => {
        const st = sn.val();
        if (!st || st.finishedAt) return;
        if (roomId !== targetRoom || myNick !== targetNick) return;
        const now = getServerTime();
        const joined = (typeof joinTs === 'number' && joinTs) ? joinTs : now;
        return set(playerRefOf(), { joinedAt: joined, lastActive: now, connected: true, clientId });
      }).catch(err => {
        console.error('presence restore failed', err);
        const s = loadSession();
        if (s && s.roomId === roomId && s.myNick === myNick) showRejoinCard(s);
      });
    }
  });
  unsubs.push(off);
}
function watchScores(){
  const off = onValue(ref(db,`rooms/${roomId}/scores`), snap=>{
    scores = snap.val()||{};
    renderPlayers();
  });
  unsubs.push(off);
}
function watchWrongs(){
  const off = onValue(ref(db,`rooms/${roomId}/wrongAnswers`), snap=>{
    wrongs = snap.val()||{};
    // 観戦モード中はUI更新を行わない（解答表示・ステータス非表示）

    // 観戦者除外: 現在の問題開始時点（currentPreStartTs）以前に参加したプレイヤーのみを対象
    // 離脱者除外: 猶予を過ぎたオフライン者は分母から外す（一時離席は在室扱い）
    let totalActive;
    if (currentPreStartTs && typeof currentPreStartTs === 'number') {
      totalActive = Object.values(players).filter(p =>
        p && isActive(p) && typeof p.joinedAt === 'number' && p.joinedAt <= currentPreStartTs
      ).length;
    } else {
      totalActive = activeNicks().length;
    }
    if(!answered && Object.keys(wrongs).length >= totalActive && totalActive > 0){
      clearTimers(); answered = true; flowStarted = false;
      revealFullQuestionAndStopSync();
      // 解決済みとしてマーキング
      resolvedIndex = idx;
      qTimerEl.textContent = '正解：' + sequence[idx].answer;
      qTimerEl.classList.add('show-answer');
      qTimerEl.style.display = 'block';
      statusEl.textContent = '全員誤答…';
      // 選択モード時は時間切れと同様に正解を赤、その他を無効化
      if (roomModeValue === 'select') lockChoicesShowingAnswer();
      aTimerEl.style.display = 'none';
      exitAnswerMode(); buzzBtn.disabled = true;
      nextBtn.disabled = false; remove(ref(db,`rooms/${roomId}/buzz`));
      startNextBtnCountdown();
    }
    updateBuzzState();
  });
  unsubs.push(off);
}
function watchSettings(){
  const off = onValue(ref(db,`rooms/${roomId}/settings`), snap=>{
    const s = snap.val()||{};
    settingsCache = s;
    roomRange.textContent = (s.chapters||[]).map(c=>CHAPTER_NAMES[c] || c).join('、');
    totalNum.textContent = s.count||0;
    // モード表示
    if (roomMode) {
      roomMode.textContent = s.mode === 'select' ? '選択' : '入力';
    }
  });
  unsubs.push(off);
}
function watchSequence(){
  const off = onValue(ref(db,`rooms/${roomId}/sequence`), snap=>{
    sequence = Object.values(snap.val()||{}).filter(x=>x);
    // 実際の合計件数をUIへ反映（settings.countより優先）
    try { if (totalNum) totalNum.textContent = sequence.length; } catch(_) {}
    // 作成時に要求数 > 実数 の場合は一度だけ通知
    try {
      const req = settingsCache && typeof settingsCache.count === 'number' ? settingsCache.count : null;
      if (!notifiedReducedCount && typeof req === 'number' && sequence.length < req) {
        showToast(`選択範囲には ${sequence.length} 問しかありませんでした。${sequence.length} 問で進行します。`, null, null, 4500);
        notifiedReducedCount = true;
      }
    } catch(_) {}
  });
  unsubs.push(off);
}
function watchIndex(){
  let firstFire = true;
  const off = onValue(ref(db,`rooms/${roomId}/currentIndex`), snap=>{
    const newIdx = snap.val()||0;
    // 初回発火（参加・復帰の直後）では進行状態を壊さない。
    // ここで無条件に wrongAnswers を消すと、途中参加者が
    // 進行中の問題の誤答履歴を全消しして早押しを復活させてしまう。
    const changed = !firstFire && newIdx !== idx;
    firstFire = false;
    idx = newIdx;
    questionLabelEl.textContent = `${TEXT.labels.questionLabelPrefix}${idx+1}${TEXT.labels.questionLabelSuffix}`;
    nextBtn.textContent = idx+1>=sequence.length?TEXT.labels.finalResult:TEXT.labels.nextQuestion;
    if (!changed) return;

    set(ref(db,`rooms/${roomId}/wrongAnswers`),null);
    // 一着判定のリセットはここで行う。出題後（showQuestion）だと端末ごとに
    // タイミングがずれ、既に確定した一着を別端末が消してしまう。
    remove(ref(db,`rooms/${roomId}/selectBuzz`));
    // ---- フラグ/状態リセット（全クライアントで新問開始時に同期） ----
    answered = false;
    flowStarted = false;
    handledCorrectFor.clear();
    clearTimers();
    didLocalWinCelebrate = false;
    // 新問開始で解決インデックスをリセット
    resolvedIndex = null;
    // 新しい問題に入ったらローカル誤答フラグを解除
    iAmWrongLocal = false;
    // 早押し関連の持ち越しを解除
    releaseBuzzOnDisconnect();
    clearStaleBuzzWatch();
    currentBuzz = null;
    lastBuzzRemain = null;
    pausedRemainingQTime = null;
    // 選択モードでは即座に旧選択肢を消しておく（カウントダウン中に表示残りを防止）
    if (roomModeValue === 'select') {
      if (choiceArea) {
        Array.from(choiceArea.children).forEach(b=>b.disabled=true);
        choiceArea.classList.add('hidden');
        choiceArea.innerHTML='';
      }
    }
    // （旧問題のタイピング進捗監視解除は clearTimers でtypeInterval解除済）
    // 参加直後のレース対策: preStart カウントダウン中に currentIndex 更新で clearTimers された場合、再度カウントダウンを開始
    // ここで最新の preStart を取得し、未経過なら startPreCountdown を呼び出す
    get(ref(db,`rooms/${roomId}/settings/preStart`)).then(s => {
      const ts = s && s.val();
      if (typeof ts === 'number') {
        const elapsed = getServerTime() - ts;
        if (elapsed < TEXT.preCountdownSec * 1000) {
          startPreCountdown(ts);
        }
      }
    }).catch(()=>{});
  });
  unsubs.push(off);
}
function watchEvents(){
  const off = onChildAdded(ref(db,`rooms/${roomId}/events`), snap=>{
    const ev = snap.val(); if(!ev || ev.timestamp <= eventCutoffTs) return;

    if(ev.correct){
      if (handledCorrectFor.has(ev.questionIndex)) return;
      handledCorrectFor.add(ev.questionIndex);
      clearTimers(); answered = true; flowStarted = false;
      releaseBuzzOnDisconnect(); clearStaleBuzzWatch(); currentBuzz = null;
      revealFullQuestionAndStopSync();
      // 解決済みとしてマーキング
      resolvedIndex = idx;
      statusEl.textContent = `${ev.nick} さんが正解！🎉`;
      // 正解演出（モード別）
      if (roomModeValue === 'input') {
        // 入力モード: 全員に丸印＋ハプティクス
        showFeedback(true);
      } else {
        // 選択モード: 全員にハプティクス、丸印は勝者のみ
        triggerHaptic(true);
        if (ev.nick === myNick) {
          showFeedback(true);
        }
      }
      // 勝者のみ紙吹雪/+1（重複防止）
      if (ev.nick === myNick && !didLocalWinCelebrate) {
        celebrateCorrect({ count: 140, pitch: 988, scoreText: '＋１' });
        didLocalWinCelebrate = true;
      }
      // スコア加算はサーバ側 Cloud Function (onCorrectEvent) が一元管理する。
      // ローカルでは加算せず、/scores の更新（watchScores）でUI同期する。
      qTimerEl.textContent = '正解：' + ev.answer;
      qTimerEl.classList.add('show-answer');
      qTimerEl.style.display = 'block';
      aTimerEl.style.display = 'none';
      exitAnswerMode();
      nextBtn.disabled = false; updateBuzzState();
      startNextBtnCountdown();
        if (roomModeValue === 'select') lockChoicesShowingAnswer();
    } else if(ev.type==='wrongGuess' || ev.type==='answerTimeout'){
      // 誤答 / 回答時間切れ: 問題再開。元の questionStart は維持し残り時間補正
      clearTimers();
      clearStaleBuzzWatch();
      if (ev.nick === myNick) releaseBuzzOnDisconnect();
      currentBuzz = null;
      let disp = ev.type==='wrongGuess'?ev.guess:'時間切れ';
      if (ev.type==='wrongGuess' && (!disp || disp==='')) disp = '空欄';
      statusEl.textContent = `${ev.nick} さんが不正解（${disp}）`;
      // 誤答演出（モード別）
      if (roomModeValue === 'input') {
        // 入力モード: 全員に×印＋ハプティクス
        showFeedback(false);
      } else {
        // 選択モード: 本人のみ
        if (ev.nick === myNick) showFeedback(false);
      }
      // 自分の誤答/時間切れなら、ローカルでも即時に誤答フラグON（再有効化を防ぐ）
      if (ev.nick === myNick) {
        iAmWrongLocal = true;
      }
      // すでに正解が確定している場合は再開しない（レース回避）
      if (handledCorrectFor.has(ev.questionIndex)) return;
      flowStarted = true;
      // 再開時刻は「イベント時刻」と「早押し時点の残り時間」から算出する。
      // どちらもサーバ由来の共有値なので、全端末で同じ questionStart になる。
      const remain = (typeof lastBuzzRemain === 'number')
        ? lastBuzzRemain
        : (pausedRemainingQTime != null ? pausedRemainingQTime : null);
      if (remain != null) {
        questionStart = computeResumeStart(ev.timestamp, remain);
        // 復帰してくる人のために、再開時刻をサーバにも残す（重複書き込みを避け本人のみ）
        if (ev.nick === myNick) {
          set(ref(db,`rooms/${roomId}/qStart/${idx}`), questionStart).catch(()=>{});
        }
      }
      currentText = sequence[idx].question;
      resumeTypewriter();   // 位置は questionStart から再計算されるので同期は不要
      lastDisplayedQSec = null; // 互換のため残すが未使用
      lastDisplayedQTenth = null;
      window._qInt = setInterval(tickQ,100);
      tickQ();
      pausedRemainingQTime = null; // 再開後クリア
      lastBuzzRemain = null;
      updateBuzzState();
    }
  });
  unsubs.push(off);
}

// 早押しが成立したときの解答UI。楽観表示と正規のイベントの両方から呼ぶので冪等にする。
let answerModeActive = false;
function focusAnswerInput(){
  const doFocus = () => {
    try {
      if (focusProxyInput) {
        try { focusProxyInput.blur(); } catch(_) {}
        if (focusProxyInput.parentNode) focusProxyInput.parentNode.removeChild(focusProxyInput);
        focusProxyInput = null;
      }
      answerInput.focus();
      try { const len = answerInput.value.length; answerInput.setSelectionRange(len, len); } catch(_){}
    } catch(_) {}
    if (window.innerWidth <= 600) {
      setTimeout(() => { try { answerInput.scrollIntoView({behavior:'smooth', block:'center'}); } catch(_) {} }, 150);
    }
  };
  if (isIOS()) requestAnimationFrame(() => setTimeout(doFocus, 0));
  else setTimeout(doFocus, 0);
}
function enterAnswerMode(buzzTime, opts){
  opts = opts || {};
  const first = !answerModeActive;
  answerModeActive = true;
  answerArea.classList.remove('hidden');
  answerBtn.disabled = false;
  if (first || opts.authoritative) startAnswerTimer(buzzTime);
  if (first && !opts.noFocus) focusAnswerInput();
}
function exitAnswerMode(){
  if (!answerModeActive) return;
  answerModeActive = false;
  clearInterval(window._aInt);
  aTimerEl.style.display = 'none';
  answerArea.classList.add('hidden');
  answerInput.value = '';
  answerBtn.disabled = true;
  if (focusProxyInput) {
    try { focusProxyInput.blur(); } catch(_) {}
    if (focusProxyInput.parentNode) focusProxyInput.parentNode.removeChild(focusProxyInput);
    focusProxyInput = null;
  }
}
// 早押しを中断状態にする（自分・他人どちらの早押しでも共通）
function enterBuzzPause(b){
  flowStarted = false;
  clearInterval(window._qInt);
  pauseTypewriter();
  pausedRemainingQTime = (typeof b.remain === 'number') ? b.remain : remainingQTime;
  statusEl.textContent = `${b.nick} さんが押しました`;
}

function watchBuzz(){
  const off = onValue(ref(db,`rooms/${roomId}/buzz`), snap=>{
    const b = snap.val();
    currentBuzz = b || null;
    if (b && typeof b.remain === 'number') lastBuzzRemain = b.remain;
    // 放置された早押しは誰でも解放できるようにする（押した人が離脱しても進行が止まらない）
    if (b) armStaleBuzzWatch(b); else clearStaleBuzzWatch();
    if (b && b.nick !== myNick) {
      // 他人が確定した。楽観表示していた場合はここで必ず引っ込める。
      exitAnswerMode();
    }
    if (b && flowStarted && !answered){
      enterBuzzPause(b);
      if (b.nick === myNick) enterAnswerMode(b.time, { authoritative: true });
      updateBuzzState();
    } else if (b && b.nick === myNick && !answered && answerModeActive) {
      // 楽観表示から確定へ。締切をサーバ側の時刻で引き直す。
      enterAnswerMode(b.time, { authoritative: true, noFocus: true });
    } else if (!b && flowStarted && !answered){
      exitAnswerMode();
      updateBuzzState();
    }
  });
  unsubs.push(off);
}
function watchPreStart(){
  let firstFire = true;
  const off = onValue(ref(db,`rooms/${roomId}/settings/preStart`), snap=>{
    const ts = snap.val();
    const isFirst = firstFire; firstFire = false;
    if (!ts) return;
    // 参加/復帰直後の初回発火で、既に出題が始まっている場合は何もしない。
    // ここで showQuestion を走らせると問題文がゼロから打ち直され、
    // 選択モードでは他の人の一着判定(selectBuzz)まで消してしまう。
    if (isFirst && (getServerTime() - ts) >= TEXT.preCountdownSec * 1000) return;
    startBtn.style.display='none';
    startPreCountdown(ts);
  });
  unsubs.push(off);
}

// ルームID生成
async function genId(){
  const roomsRef = ref(db,'rooms');
  let id, exists=true;
  while(exists){
  id = String(1000 + Math.floor(Math.random()*9000));
    exists = (await get(child(roomsRef,id))).exists();
  }
  return id;
}

// =====================================================================
// プレゼンス（在室状態）
// 削除ではなく connected フラグで表現し、猶予期間内は在室扱いにする。
// これにより「アプリを切り替えただけ」でルームから消えなくなる。
// =====================================================================
function playerRefOf(nick){
  return ref(db, `rooms/${roomId}/players/${nick || myNick}`);
}

// プレイヤーがまだ在室しているとみなすか（DBは書き換えず、読み取り側で判定）
function isActive(p){
  if (!p) return false;
  if (p.connected === false) {
    const la = typeof p.lastActive === 'number' ? p.lastActive : 0;
    return (getServerTime() - la) < PRESENCE_GRACE_MS;
  }
  return true;
}
function activeNicks(){
  return Object.keys(players).filter(n => isActive(players[n]));
}

// ハートビート（lastActive 更新）＋ セッションの鮮度更新
function startHeartbeat(){
  if (heartbeatTimer) clearInterval(heartbeatTimer);
  heartbeatTimer = setInterval(()=>{
    if (!roomId || !myNick) return;
    update(playerRefOf(), { lastActive: getServerTime(), connected: true, clientId }).catch(()=>{});
    touchSession();
  }, 15000); // 15秒間隔
}
function stopHeartbeat(){
  if (heartbeatTimer) { clearInterval(heartbeatTimer); heartbeatTimer = null; }
}

// .info/connected を起点に在室情報と onDisconnect を張り直す（Firebase標準のプレゼンス）
// 画面を見たままネットワークだけ瞬断した場合もここで復帰できる。
function attachPresence(){
  detachPresence();
  if (!roomId || !myNick) return;
  const connRef = ref(db, `rooms/${roomId}/players/${myNick}/connected`);
  let first = true;
  presenceUnsub = onValue(ref(db, '.info/connected'), snap => {
    if (!snap.val()) return;              // 切断中は何もしない
    try { onDisconnect(connRef).set(false); } catch(_) {}
    update(playerRefOf(), { connected: true, lastActive: getServerTime(), clientId }).catch(()=>{});
    if (first) { first = false; return; }
    // 2回目以降＝再接続。進行度を取り直す。
    resyncGameState('reconnect');
  });
}
function detachPresence(){
  if (presenceUnsub) { try { presenceUnsub(); } catch(_) {} presenceUnsub = null; }
}

// =====================================================================
// ウォッチャーの一括着脱（復帰時の二重登録を防ぐ）
// =====================================================================
function attachWatchers(){
  detachWatchers();
  watchPlayers(); watchScores(); watchWrongs();
  watchSettings(); watchSequence(); watchIndex();
  watchEvents(); watchBuzz(); watchPreStart();
}
function detachWatchers(){
  try { unsubs.forEach(u=>{ if (typeof u === 'function') u(); }); } catch(e) {}
  unsubs = [];
}

// =====================================================================
// 早押しロックの保護
// 早押しした人が離脱したまま戻らないと全員が待たされるため、
// ① 切断時に buzz を自動解放 ② 期限超過なら誰でも解放できる、の二段構え。
// =====================================================================
async function claimBuzzOnDisconnect(buzzRef){
  try {
    myBuzzOnDisconnect = onDisconnect(buzzRef);
    await myBuzzOnDisconnect.remove();
  } catch(_) { myBuzzOnDisconnect = null; }
}
async function releaseBuzzOnDisconnect(){
  const od = myBuzzOnDisconnect;
  myBuzzOnDisconnect = null;
  if (!od) return;
  try { await od.cancel(); } catch(_) {}
}
function clearStaleBuzzWatch(){
  if (staleBuzzTimer) { clearTimeout(staleBuzzTimer); staleBuzzTimer = null; }
}
function armStaleBuzzWatch(b){
  clearStaleBuzzWatch();
  if (!b || typeof b.time !== 'number' || !roomId) return;
  const wait = Math.max(1000, (b.time + STALE_BUZZ_MS) - getServerTime() + 500);
  staleBuzzTimer = setTimeout(async () => {
    staleBuzzTimer = null;
    try {
      const cur = (await get(ref(db, `rooms/${roomId}/buzz`))).val();
      if (!cur || cur.time !== b.time) return;        // 既に解放/交代済み
      if (getServerTime() - cur.time < STALE_BUZZ_MS) { armStaleBuzzWatch(cur); return; }
      await releaseStaleBuzz(cur);
    } catch(_) {}
  }, wait);
}
// 放置された buzz を1回だけ時間切れとして確定させる
async function releaseStaleBuzz(b){
  const buzzRef = ref(db, `rooms/${roomId}/buzz`);
  let committed = false;
  try {
    const r = await runTransaction(buzzRef, cur => {
      if (cur && cur.time === b.time) return null;   // 解放
      return;                                        // 変わっていたら中断
    }, { applyLocally: false });
    committed = !!(r && r.committed);
  } catch(_) { return; }
  if (!committed) return;
  try {
    await push(ref(db, `rooms/${roomId}/events`), {
      nick: b.nick,
      correct: false,
      guess: '時間切れ',
      answer: (sequence[idx] && sequence[idx].answer) || '',
      type: 'answerTimeout',
      questionIndex: idx,
      timestamp: getServerTime()
    });
    await set(ref(db, `rooms/${roomId}/wrongAnswers/${b.nick}`), true);
  } catch(_) {}
}

// 誤答からの再開時刻を全端末で揃えるための共通計算
function computeResumeStart(evTimestamp, remainSec){
  const base = (typeof evTimestamp === 'number' && evTimestamp) ? evTimestamp : getServerTime();
  return base - (TEXT.questionTimeLimit - remainSec) * 1000;
}

// =====================================================================
// 進行度の再同期
// どの復帰経路から来ても同じ状態になるよう、サーバの値だけからUIを組み直す。
// =====================================================================
async function resyncGameState(reason){
  if (!roomId || !myNick) return false;
  if (resyncPending) return false;
  resyncPending = true;
  const cutoff = getServerTime();
  try {
    const base = `rooms/${roomId}`;
    // sequence は作成後に変化しないので、未取得のときだけ読む（復帰のたびの再DLを避ける）
    const needSequence = !sequence.length;
    const reads = [
      get(ref(db, `${base}/settings`)),
      get(ref(db, `${base}/currentIndex`)),
      get(ref(db, `${base}/buzz`)),
      get(ref(db, `${base}/wrongAnswers`)),
      get(ref(db, `${base}/qStart`)),
      get(ref(db, `${base}/players`)),
      get(ref(db, `${base}/awards`)),
      needSequence ? get(ref(db, `${base}/sequence`)) : Promise.resolve(null)
    ];
    const [sVal, iVal, bVal, wVal, qVal, plVal, awVal, seqVal] =
      (await Promise.all(reads)).map(r => (r ? r.val() : null));

    const s = sVal || {};
    if (s.finishedAt) {
      // 離れている間に終了していた場合は結果画面へ送る
      if (seqVal) sequence = Object.values(seqVal).filter(Boolean);
      detachWatchers(); detachPresence(); clearTimers(); stopHeartbeat();
      if (!quizAppDiv.classList.contains('hidden')) { showResults(); }
      else { clearSession(); }
      return false;
    }

    eventCutoffTs = cutoff;
    settingsCache = s;
    roomModeValue = s.mode || roomModeValue || 'input';
    if (seqVal) sequence = Object.values(seqVal).filter(Boolean);
    try { if (totalNum && sequence.length) totalNum.textContent = sequence.length; } catch(_) {}
    wrongs = wVal || {};
    if (plVal) { players = plVal; renderPlayers(); }

    idx = (typeof iVal === 'number') ? iVal : 0;
    try { if (currentNum) currentNum.textContent = idx + 1; } catch(_) {}
    if (questionLabelEl) {
      questionLabelEl.textContent = `${TEXT.labels.questionLabelPrefix}${idx+1}${TEXT.labels.questionLabelSuffix}`;
    }
    if (nextBtn) nextBtn.textContent = idx+1 >= sequence.length ? TEXT.labels.finalResult : TEXT.labels.nextQuestion;

    const preTs = (typeof s.preStart === 'number') ? s.preStart : null;
    currentPreStartTs = preTs;

    clearTimers();
    clearStaleBuzzWatch();
    exitAnswerMode();
    // 途中参加で観戦中だった場合、同じ問題の間は観戦のままにする。
    // （ゲーム開始から居た人の復帰は観戦にしない）
    const keepSpectating = spectatorUntilNext && preStartSkipTs != null && preStartSkipTs === s.preStart;
    if (!keepSpectating) {
      hideSpectatorBanner();
      spectatorUntilNext = false;
      preStartSkipTs = null;
    }

    // --- A: まだ出題前 ---
    if (!preTs || !sequence.length) {
      hideSpectatorBanner(); spectatorUntilNext = false; preStartSkipTs = null;
      flowStarted = false; answered = false;
      iAmWrongLocal = false; pausedRemainingQTime = null; lastBuzzRemain = null;
      showPreGameUI(s);
      return true;
    }

    const preElapsed = getServerTime() - preTs;

    // --- B: カウントダウン中 ---
    if (preElapsed < TEXT.preCountdownSec * 1000) {
      hideSpectatorBanner(); spectatorUntilNext = false; preStartSkipTs = null;
      if (startBtn) startBtn.style.display = 'none';
      startPreCountdown(preTs);
      return true;
    }

    // --- C: 出題中 or 解決済み ---
    const winner = (awVal && awVal[idx]) || null;
    const qStart = (qVal && typeof qVal[idx] === 'number')
      ? qVal[idx]
      : (preTs + TEXT.preCountdownSec * 1000);
    questionStart = qStart;
    currentText = (sequence[idx] && sequence[idx].question) || '';
    iAmWrongLocal = !!wrongs[myNick];
    pausedRemainingQTime = null;

    const timedOut = (getServerTime() - qStart) >= TEXT.questionTimeLimit * 1000;
    // 全員誤答で決着していたかどうかも見る（watchWrongs と同じ条件）
    const eligible = Object.keys(players).filter(n => {
      const p = players[n];
      if (!p || !isActive(p)) return false;
      if (typeof p.joinedAt === 'number') return p.joinedAt <= preTs;
      return true;
    });
    const allWrong = eligible.length > 0 && eligible.every(n => !!wrongs[n]);
    restoreQuestionCard();

    if (keepSpectating) showSpectatorBanner();

    if (winner || timedOut || allWrong) {
      // 決着済み：正解表示と「次へ」まで進める
      answered = true; flowStarted = false;
      resolvedIndex = idx;
      handledCorrectFor.add(idx);
      revealFullQuestionAndStopSync();
      qTimerEl.textContent = '正解：' + ((sequence[idx] && sequence[idx].answer) || '');
      qTimerEl.classList.add('show-answer');
      qTimerEl.style.display = 'block';
      statusEl.textContent = winner ? `${winner.nick} さんが正解！🎉`
        : (timedOut ? '時間切れ！' : '全員誤答…');
      exitAnswerMode();
      aTimerEl.style.display = 'none';
      buzzBtn.disabled = true;
      if (roomModeValue === 'select') { renderChoices(); lockChoicesShowingAnswer(); }
      // 復帰した端末が勝手に次の問題へ進めないよう、自動カウントダウンはしない。
      // （その場に居た端末が進めるか、本人が手動で押す）
      clearNextBtnCountdown();
      nextBtn.disabled = false;
      return true;
    }

    // --- 進行中 ---
    answered = false;
    restoreQuestionText();
    if (roomModeValue === 'select') renderChoices();

    currentBuzz = bVal || null;
    if (currentBuzz && currentBuzz.nick) {
      // 誰かが早押し中：問題は停止したまま
      flowStarted = false;
      if (typeof currentBuzz.remain === 'number') lastBuzzRemain = currentBuzz.remain;
      pausedRemainingQTime = (typeof currentBuzz.remain === 'number')
        ? currentBuzz.remain
        : Math.max(0, TEXT.questionTimeLimit - (getServerTime() - qStart) / 1000);
      qTimerEl.style.display = 'block';
      statusEl.textContent = currentBuzz.nick === myNick
        ? `${myNick} さんが押しました`
        : `${currentBuzz.nick} さんが押しました`;
      if (currentBuzz.nick === myNick && !iAmWrongLocal) {
        enterAnswerMode(currentBuzz.time, { authoritative: true, noFocus: true });
      } else {
        exitAnswerMode();
      }
      if (roomModeValue === 'select') {
        Array.from(choiceArea.children).forEach(b => b.disabled = true);
      }
      armStaleBuzzWatch(currentBuzz);
    } else {
      // 通常進行：タイマーとタイプを再開
      flowStarted = true;
      exitAnswerMode();
      qTimerEl.classList.remove('show-answer');
      qTimerEl.style.display = 'block';
      lastDisplayedQTenth = null;
      window._qInt = setInterval(tickQ, 100);
      tickQ();
      resumeTypewriter();
    }
    nextBtn.disabled = true;
    updateBuzzState();
    return true;
  } catch(e) {
    console.error('resyncGameState failed', reason, e);
    showToast('進行状況の同期に失敗しました', null, null, 2500);
    return false;
  } finally {
    resyncPending = false;
  }
}

// 出題前（待機中）のUI
function showPreGameUI(s){
  try { questionCardBlock.classList.add('hidden'); } catch(_) {}
  setStatus('');
  qTimerEl.style.display = 'none';
  aTimerEl.style.display = 'none';
  exitAnswerMode();
  if (nextBtn) nextBtn.disabled = true;
  applyModeUI();
  if (roomModeValue !== 'select') {
    buzzBtn.disabled = true;
    buzzBtn.classList.add('disabled-btn');
  }
  if (s && s.host === myNick) showHostStartUI(); else showWaitCaption();
}

// 出題中の問題カードを表示状態に戻す
function restoreQuestionCard(){
  questionCardBlock.classList.remove('hidden');
  if (questionLabelEl) questionLabelEl.style.visibility = 'visible';
  const pre = document.getElementById('pre-countdown');
  if (pre) { pre.style.display = 'none'; preCd.textContent = ''; }
  questionEl.style.display = 'block';
  questionEl.style.visibility = 'visible';
  if (startBtn) startBtn.style.display = 'none';
  if (document.getElementById('host-caption')) { try { hostCaption.remove(); } catch(_) {} }
  if (document.getElementById('host-caption-wrap')) {
    const w = document.getElementById('host-caption-wrap'); if (w) w.remove();
  }
  if (document.getElementById('wait-caption')) { try { waitCaption.remove(); } catch(_) {} }
}

// 問題文の表示位置を復元する（経過時間から決まるので同期値は不要）。
function restoreQuestionText(){
  paintTypewriter();
}

// =====================================================================
// ページのライフサイクル
// unload は発火保証がなく、ブラウザによっては bfcache（ページ復元）を阻害する。
// pagehide / pageshow に置き換え、復元されたときは破棄せず再同期する。
// =====================================================================
document.addEventListener('visibilitychange', ()=>{
  if (!roomId || !myNick) return;
  if (document.visibilityState === 'visible') {
    const awayMs = lastHiddenAt ? (getServerTime() - lastHiddenAt) : 0;
    lastHiddenAt = 0;
    update(playerRefOf(), { lastActive: getServerTime(), connected: true, clientId }).catch(()=>{});
    touchSession();
    if (awayMs > 3000) {
      // 実際に離れていた場合だけ再同期し、そのときだけ通知する
      resyncGameState('visible').then(ok => {
        if (ok) showToast('ゲームに復帰しました', null, null, 1800);
      });
    } else if (flowStarted && !answered) {
      // 短時間の切り替え：タイマーの更新頻度だけ戻す
      try { clearInterval(window._qInt); } catch(e) {}
      window._qInt = setInterval(tickQ, 100);
      try { tickQ(); } catch(e) {}
    }
  } else {
    lastHiddenAt = getServerTime();
    // バックグラウンド時: タイマーの更新頻度を軽減
    if (flowStarted && !answered) {
      try { clearInterval(window._qInt); } catch(e) {}
      window._qInt = setInterval(tickQ, 500);
    }
  }
});

window.addEventListener('pagehide', e => {
  if (e.persisted) return;   // bfcache に入るだけ。何も壊さない。
  detachWatchers();
  detachPresence();
  clearStaleBuzzWatch();
  clearTimers();
  stopHeartbeat();
});

window.addEventListener('pageshow', e => {
  if (!e.persisted) return;  // bfcache からの復元
  if (!roomId || !myNick) return;
  attachPresence();
  startHeartbeat();
  resyncGameState('bfcache');
});

// =====================================================================
// ホームに戻る（明示的な離脱）
// 通常の退席（タブを閉じる・瞬断など）と違い、この経路で戻った場合は
// セッションを破棄するため自動復帰も復帰カードも出さない。
// =====================================================================
async function leaveToHome(){
  if (!roomId || !myNick) { location.reload(); return; }
  if (!confirm(TEXT.labels.homeConfirm)) return;
  const targetRoom = roomId, targetNick = myNick;

  allowUnload = true;   // beforeunload の離脱確認は出さない
  // 自動復帰の材料を先に消す（復帰カードも出さない）
  clearSession();

  // 監視・タイマーを先に止める（watchPlayers による自分のノード自動復元を防ぐ）
  detachWatchers();
  detachPresence();
  clearStaleBuzzWatch();
  clearTimers();
  stopHeartbeat();

  showLoading('ホームに戻ります...');
  // 切断時の onDisconnect（connected:false / buzz 解放）はもう不要
  try { onDisconnect(ref(db, `rooms/${targetRoom}/players/${targetNick}/connected`)).cancel(); } catch(_) {}
  await releaseBuzzOnDisconnect();

  // 自分が早押しを保持したままなら解放して、残った人を待たせない
  try {
    if (currentBuzz && currentBuzz.nick === targetNick) {
      await runTransaction(ref(db, `rooms/${targetRoom}/buzz`), cur => {
        if (cur && cur.nick === targetNick) return null;  // 解放
        return;                                           // 変わっていたら中断
      }, { applyLocally: false });
    }
  } catch(_) {}

  // 在室リストから抜ける（全員誤答判定の分母から即座に外れる）
  try { await remove(ref(db, `rooms/${targetRoom}/players/${targetNick}`)); } catch(_) {}

  roomId = ''; myNick = '';
  location.reload();
}
if (homeBtn) {
  homeBtn.addEventListener('click', () => { leaveToHome(); });
}

// ページロード時に復帰を試行（本人だと確認できれば自動、できなければ常設カード）
try { setTimeout(() => { tryAutoRejoin(); }, 200); } catch(_) {}

// ルーム作成
createBtn.addEventListener('click',async()=>{
  const chs=[...chapterCbs].filter(cb=>cb.checked).map(cb=>+cb.value);
  const cnt=parseInt(roomCount.value,10);
  // モード取得（スイッチ：未チェック→input、チェック→select）
  const mode = modeSwitch && modeSwitch.checked ? 'select' : 'input';
  roomModeValue = mode; // ここでグローバル変数にもセット
  if(!chs.length||cnt<1){ alert('範囲と数を指定'); return; }
  const nick = await showNicknameModal();
  if(!nick) return;
  await waitServerTime();
  myNick=nick; joinTs=getServerTime(); roomId=await genId();
  showLoading('ルームを作成中...');
  createBtn.disabled = true;
  try {
    // 先に暫定の設定（要求件数で）を書き込む
    await set(ref(db,`rooms/${roomId}/settings`),{chapters:chs,count:cnt,mode:mode,createdAt:getServerTime(),host:nick});
    // Cloud Functionsでシーケンス生成（章フィルタ＋件数＋選択モード）
    try {
      const resp = await fetch(`${FUNCTIONS_BASE}/generateSequence`, {
        method: 'POST', headers: { 'Content-Type':'application/json' },
        body: JSON.stringify({ chapters: chs, count: cnt, mode })
      });
      const data = await resp.json();
      if (!resp.ok) throw new Error(data && data.error || 'generateSequence failed');
      sequence = Array.isArray(data.sequence) ? data.sequence : [];
    } catch(e) {
      // フォールバック: RTDBから生成
      const snap = await get(ref(db,'questions'));
      const all = Object.values(snap.val()||{}).filter(Boolean);
      const pool = all.filter(q=>chs.includes(+q.chapter));
      for(let i=pool.length-1;i>0;i--){
        const j=Math.floor(Math.random()*(i+1));
        [pool[i],pool[j]]=[pool[j],pool[i]];
      }
      if (mode === 'select') {
        pool.forEach(q => {
          let order = [0,1,2,3,4];
          for (let i = order.length - 1; i > 0; i--) {
            const j = Math.floor(Math.random() * (i + 1));
            [order[i], order[j]] = [order[j], order[i]];
          }
          q.choicesOrder = order;
        });
      }
      sequence = pool.slice(0,cnt);
    }
    // 実際に用意できた件数
    const actualCount = Array.isArray(sequence) ? sequence.length : 0;
    // 不足している場合はホストに通知
    if (actualCount < cnt) {
      try { showToast(`選択範囲には ${actualCount} 問しかありませんでした。${actualCount} 問で進行します。`, null, null, 4500); } catch(_) {}
    }
    await set(ref(db,`rooms/${roomId}/sequence`),sequence);
    await set(ref(db,`rooms/${roomId}/currentIndex`),0);
    // settings.count を実際の件数で上書き（UI整合性のため）
    try { await update(ref(db,`rooms/${roomId}/settings`), { count: actualCount }); } catch(_) {}
    const playerRef = ref(db,`rooms/${roomId}/players/${myNick}`);
    await set(playerRef,{joinedAt:joinTs,lastActive:getServerTime(),connected:true,clientId});
    // scores はサーバのみが更新（UI側では未定義を0扱い）
    eventCutoffTs = joinTs;
    attachPresence();
    startHeartbeat();
    homeDiv.classList.add('hidden'); quizAppDiv.classList.remove('hidden');
    hideRejoinCard();
    currentRoom.textContent=roomId; startBtn.style.display='block';
  saveSession();
  showToast(`ルーム ${roomId} を作成しました`, null, null, 2000);
  } catch(err) {
    alert('ルーム作成に失敗しました。時間をおいて再度お試しください。');
    return;
  } finally {
    hideLoading();
    createBtn.disabled = false;
  }
  // ホスト用キャプションとボタンを縦並び中央揃えでラップ
  let wrap = document.getElementById('host-caption-wrap');
  if (!wrap) {
    wrap = document.createElement('div');
    wrap.id = 'host-caption-wrap';
    wrap.style = 'display:flex; flex-direction:column; align-items:center; width:100%';
    startBtn.parentNode.insertBefore(wrap, startBtn);
  }
  // 必ずキャプション→ボタンの順でラップ内に配置
  if (!wrap.contains(hostCaption)) wrap.appendChild(hostCaption);
  if (!wrap.contains(startBtn)) wrap.appendChild(startBtn);
  // 参加者用キャプションは消す
  if (document.getElementById('wait-caption')) {
    waitCaption.remove();
  }
  // ここで選択モードならbuzzBtnを非表示
  if (roomModeValue === 'select') {
    buzzBtn.style.display = 'none';
    choiceArea.classList.add('hidden');
  } else {
    buzzBtn.style.display = '';
    choiceArea.classList.add('hidden');
  }
  attachWatchers();
});

// ルーム参加
joinRoomBtn.addEventListener('click',async()=>{
  const inputId=roomIdInput.value.trim();
  if(!inputId){ alert('ルームIDを入力してください'); return; }
  await waitServerTime();
  const snap=await get(child(ref(db,'rooms'),inputId));
  if(!snap.exists()){ alert('ルームが存在しません'); return; }
  roomId=inputId;
  // ルームのモード取得
  const settingsSnap = await get(ref(db,`rooms/${roomId}/settings`));
  const settingsObj = settingsSnap.val() || {};
  // 参加可否: 終了済みは参加不可。
  // 「無人」は全員が一時的にバックグラウンドにいるだけの場合があるため、
  // 猶予内にアクティブだった人が誰もいないときだけ拒否する。
  const playersSnap0 = await get(ref(db,`rooms/${roomId}/players`));
  const playersObj0 = playersSnap0.val() || {};
  const liveCount0 = Object.values(playersObj0).filter(p => isActive(p)).length;
  if (settingsObj.finishedAt) {
    alert('このルームには参加できません（終了済み）');
    return;
  }
  if (Object.keys(playersObj0).length === 0 && liveCount0 === 0) {
    alert('このルームには参加できません（無人）');
    return;
  }
  roomModeValue = settingsObj.mode || 'input';

  // 自分の端末が既にこのルームに居るなら、名前を聞かずそのまま復帰させる
  const mineNick = Object.keys(playersObj0).find(n => playersObj0[n] && playersObj0[n].clientId === clientId);
  if (mineNick) {
    await rejoinWithSession({
      roomId, myNick: mineNick,
      joinedAt: playersObj0[mineNick] && playersObj0[mineNick].joinedAt
    });
    return;
  }

  const nick = await showNicknameModal();
  if(!nick) return;
  // ここから参加処理はローディングを表示
  showLoading('ルームへ参加中...');
  joinRoomBtn.disabled = true;
  try {
    // 既存参加者と同じニックネームは不可（ただし同じ端末なら本人として引き継ぐ）
    const playersSnap = await get(ref(db,`rooms/${roomId}/players`));
    const playersObj = playersSnap.val() || {};
    const existing = playersObj[nick];
    if (existing) {
      if (!existing.clientId || existing.clientId === clientId) {
        hideLoading();
        await rejoinWithSession({ roomId, myNick: nick, joinedAt: existing.joinedAt });
        return;
      }
      alert('その名前は既に使われています。他の名前を入力してください。');
      return;
    }

    myNick=nick; joinTs=getServerTime(); eventCutoffTs = joinTs;
    const playerRef = ref(db,`rooms/${roomId}/players/${myNick}`);
    await set(playerRef,{joinedAt:joinTs,lastActive:getServerTime(),connected:true,clientId});
    // scores はサーバのみが更新
    attachPresence();
    startHeartbeat();
    homeDiv.classList.add('hidden'); quizAppDiv.classList.remove('hidden');
    hideRejoinCard();
    currentRoom.textContent=roomId;
    settingsCache = settingsObj;
    applyModeUI();

    // 途中参加判定: preStart の状態に応じてUI分岐
    const preStartSnap = await get(ref(db,`rooms/${roomId}/settings/preStart`));
    const preTs = preStartSnap.val();
    const elapsedFromPre = preTs ? (getServerTime() - preTs) : null;
    // 現在の問題番号を先に取得（観戦UIのラベルに反映）
    try {
      const idxSnap = await get(ref(db,`rooms/${roomId}/currentIndex`));
      const v = idxSnap && idxSnap.val();
      if (typeof v === 'number') idx = v;
    } catch(_) {}

    if (preTs && elapsedFromPre >= TEXT.preCountdownSec * 1000) {
      // 進行中の問題には解答できないが、観戦としては最後まで見せる
      spectatorUntilNext = true;
      preStartSkipTs = preTs;
      showSpectatorBanner();
      if (document.getElementById('wait-caption')) { try { waitCaption.remove(); } catch(_){} }
      if (document.getElementById('host-caption')) { try { hostCaption.remove(); } catch(_){} }
      // 表示の組み立ては resyncGameState に任せる（進行中の問題文・残り時間まで追える）
    } else if (preTs && elapsedFromPre < TEXT.preCountdownSec * 1000) {
      // カウントダウン進行中に参加（観戦ではない）
      spectatorUntilNext = false;
      if (startBtn) startBtn.style.display = 'none';
      try { clearInterval(window._preInt); } catch(_) {}
      startPreCountdown(preTs);
    } else {
      // 出題前
      spectatorUntilNext = false;
      showPreGameUI(settingsObj);
    }

    attachWatchers();
    if (spectatorUntilNext) await resyncGameState('join-spectate');
    saveSession();
    showToast(`ルーム ${roomId} に参加しました`, null, null, 2000);
  } catch (err) {
    console.error('join failed', err);
    alert('ルーム参加に失敗しました。時間をおいて再度お試しください。');
    return;
  } finally {
    hideLoading();
    joinRoomBtn.disabled = false;
  }
});

// プリカウント→出題
startBtn.addEventListener('click',async()=>{
  const now=getServerTime(); await set(ref(db,`rooms/${roomId}/settings/preStart`),now);
});

// ローカルで残り秒数を先行描画
function updateLocalCountdown(startTs) {
  const tick = () => {
    const rem = TEXT.preCountdownSec - Math.floor((getServerTime() - startTs) / 1000);
    if (rem > 0) preCd.textContent = rem;
    else {
      clearInterval(window._preInt);
      preCd.textContent = '';
  }
  };
  tick();
  window._preInt = setInterval(tick, 200);
}

function startPreCountdown(startTs){
  // 新しい preStart を受信したら、観戦モードを解除（1問だけスキップの想定）
  if (spectatorUntilNext && preStartSkipTs && startTs !== preStartSkipTs) {
    spectatorUntilNext = false;
    preStartSkipTs = null;
    try { hideSpectatorBanner(); } catch(_) {
      const b = document.getElementById('spectator-banner'); if (b) b.remove();
    }
    const preEl = document.getElementById('pre-countdown');
    if (preEl) { preEl.style.display = 'block'; preCd.textContent = ''; }
  }
  // この問題の開始基準時刻を保存
  currentPreStartTs = startTs;
  const elapsedMs = getServerTime() - startTs;
  const remain = TEXT.preCountdownSec - Math.floor(elapsedMs / 1000);

  // すでにカウントダウンが終わっている（=出題中）の場合は、カウントダウンUIを経由せずに即座に問題表示へ遷移
  if (remain <= 0) {
    // 既存の進行を壊さないよう、プリカウントのみに関わるUIだけ触る
    try { clearInterval(window._preInt); } catch(e) {}
    const preEl = document.getElementById('pre-countdown');
    if (preEl) { preEl.style.display = 'block'; preCd.textContent = ''; }
    // 観戦中でも問題カードは表示し、番号と観戦バナーを出す
    questionCardBlock.classList.remove('hidden');
    if (questionLabelEl) {
      questionLabelEl.style.visibility = 'visible';
      questionLabelEl.textContent = `${TEXT.labels.questionLabelPrefix}${idx+1}${TEXT.labels.questionLabelSuffix}`;
    }
    if (spectatorUntilNext) { try { showSpectatorBanner(); } catch(_) {} }
    questionStart = startTs + TEXT.preCountdownSec * 1000;
    flowStarted = true;
    showQuestion();
    return;
  }

  // ここからはカウントダウン中の通常ルート
  clearTimers(); flowStarted=false; answered=false; pausedRemainingQTime = null;
  statusEl.textContent=''; exitAnswerMode();
  qTimerEl.style.display='none'; aTimerEl.style.display='none'; questionEl.style.visibility='hidden';
  questionCardBlock.classList.remove('hidden');
  questionLabelEl.style.visibility='visible';
  questionLabelEl.textContent = `${TEXT.labels.questionLabelPrefix}${idx+1}${TEXT.labels.questionLabelSuffix}`;
  if (currentNum) currentNum.textContent = idx + 1;
  document.getElementById('pre-countdown').style.display = 'block';
  document.getElementById('question').style.display = 'none';
  document.getElementById('question-timer').style.display = 'none';
  nextBtn.disabled=true;
  if (roomModeValue === 'select' && choiceArea) {
    Array.from(choiceArea.children).forEach(b=>b.disabled=true);
    choiceArea.classList.add('hidden');
  }
  if (document.getElementById('host-caption')) hostCaption.remove();
  if (document.getElementById('host-caption-wrap')) document.getElementById('host-caption-wrap').remove();
  if (document.getElementById('wait-caption')) waitCaption.remove();

  // 数字の描画は200ms刻みで十分だが、出題への切り替えだけは正確な時刻で予約する。
  // tick 任せだと端末ごとに最大200msずれ、問題が見え始める瞬間が揃わない。
  const tick = () => {
    const r = TEXT.preCountdownSec - Math.floor((getServerTime() - startTs) / 1000);
    preCd.textContent = r > 0 ? r : '';
  };
  tick();
  window._preInt = setInterval(tick, 200);
  clearTimeout(window._preTo);
  const fireAt = startTs + TEXT.preCountdownSec * 1000;
  window._preTo = setTimeout(() => {
    clearInterval(window._preInt);
    preCd.textContent = '';
    questionStart = fireAt;
    flowStarted = true;
    showQuestion();
  }, Math.max(0, fireAt - getServerTime()));
}

// タイプ制御
let typePos=0, currentText='';
function showQuestion(){
  currentText = (sequence[idx] && sequence[idx].question) || '';
  questionEl.style.visibility = 'visible';
  questionCardBlock.classList.remove('hidden');
  document.getElementById('pre-countdown').style.display = 'none';
  document.getElementById('question').style.display = 'block';
  const qt = document.getElementById('question-timer');
  // タイマーの表示を初期化（前問の正解表示や残秒が残らないように）
  qt.classList.remove('show-answer');
  qt.textContent = `${TEXT.labels.timeoutLabel}${TEXT.questionTimeLimit}${TEXT.labels.secondsSuffix}`;
  qt.style.display = 'block';

  // ホストだけが、この問題の開始時刻をサーバに残す。
  // 復帰時の復元と、ログの解答時間集計に使う（1問1回だけの書き込み）。
  const isHost = settingsCache && settingsCache.host && myNick === settingsCache.host;
  if (isHost && roomId) {
    set(ref(db, `rooms/${roomId}/qStart/${idx}`), questionStart).catch(()=>{});
  }

  // 問題文はサーバ時刻から位置を決めて描画する（全端末で必ず一致する）
  typePos = 0;
  questionEl.textContent = '';
  startTypewriter();

  clearInterval(window._qInt); qTimerEl.style.display='block';
  // questionStart は startPreCountdown が preStart から算出済み。
  // ここで現在時刻に置き換えると、遅れて合流した端末だけ時間が長くなってしまう。
  if (!questionStart) questionStart = getServerTime();
  remainingQTime=TEXT.questionTimeLimit; pausedRemainingQTime = null;
  lastDisplayedQSec = null; // 互換のため残すが未使用
  lastDisplayedQTenth = null;
  window._qInt=setInterval(tickQ,100);
  tickQ();
  // --- 選択モード分岐 ---
  if (roomModeValue === 'select') {
    buzzBtn.style.display = 'none';
    answerArea.classList.add('hidden');
    renderChoices();
  } else {
    buzzBtn.style.display = '';
    choiceArea.classList.add('hidden');
    updateBuzzState();
  }
}

// 選択肢を描画する（出題時と復帰時の両方から呼ぶ）
function renderChoices(){
  if (roomModeValue !== 'select' || !choiceArea) return;
  const q = sequence[idx];
  if (!q) return;
  choiceArea.classList.remove('hidden');
  const baseChoices = [q.answer, q.ng1, q.ng2, q.ng3, q.ng4];
  let choices;
  if (q.choicesOrder && Array.isArray(q.choicesOrder) && q.choicesOrder.length === 5) {
    choices = q.choicesOrder.map(i => ({c: baseChoices[i], i}));
  } else {
    // 旧データ用: ローカルシャッフル
    choices = baseChoices.map((c, i) => ({c, i}));
    for (let i = choices.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [choices[i], choices[j]] = [choices[j], choices[i]];
    }
  }
  choiceArea.innerHTML = '';
  choices.forEach(obj => {
    const btn = document.createElement('button');
    btn.className = 'btn-primary';
    btn.textContent = obj.c;
    btn.style.fontSize = '1.15em';
    btn.style.padding = '0.7em 0.5em';
    btn.style.marginBottom = '0.2em';
    btn.dataset.choiceIdx = obj.i;
    btn.dataset.isAnswer = (obj.i === 0 ? '1' : '0');
    btn.disabled = (spectatorUntilNext || iAmWrongLocal) ? true : false;
    btn.addEventListener('click', async () => {
      if (btn.disabled) return;
      if (btn.dataset.isAnswer === '1') {
        // 正解ボタン: 一着判定（トランザクション）
        const selectRef = ref(db, `rooms/${roomId}/selectBuzz`);
        btn.disabled = true;
        btn.classList.add('disabled-btn');
        statusEl.textContent = `${myNick} さんが押しました（判定中…）`;
        await runTransaction(selectRef, current => {
          if (current === null) {
            return { nick: myNick, time: getServerTime() };
          }
          return;
        }, { applyLocally: false }).then(async result => {
          if (!result.committed) {
            get(selectRef).then(snap => {
              const selectData = snap.val();
              const who = selectData && selectData.nick ? selectData.nick : '他のプレイヤー';
              statusEl.textContent = `${who} さんが先に押しました…`;
              Array.from(choiceArea.children).forEach(b => b.disabled = true);
            });
          } else {
            statusEl.textContent = `正解！🎉`;
            Array.from(choiceArea.children).forEach(b => b.disabled = true);
            await push(ref(db, `rooms/${roomId}/events`), {
              nick: myNick,
              correct: true,
              guess: btn.textContent,
              answer: sequence[idx].answer,
              questionIndex: idx,
              timestamp: getServerTime(),
              type: 'selectCorrect'
            });
          }
        });
      } else {
        // 誤答ボタン
        btn.disabled = true;
        btn.classList.add('disabled-btn');
        btn.style.background = '#e0e0e0';
        btn.style.color = '#888';
        iAmWrongLocal = true;
        Array.from(choiceArea.children).forEach(b => b.disabled = true);
        await push(ref(db, `rooms/${roomId}/events`), {
          nick: myNick,
          correct: false,
          guess: btn.textContent,
          answer: sequence[idx].answer,
          questionIndex: idx,
          timestamp: getServerTime(),
          type: 'wrongGuess'
        });
        await set(ref(db,`rooms/${roomId}/wrongAnswers/${myNick}`), true);
      }
    });
    choiceArea.appendChild(btn);
  });
}
// =====================================================================
// 問題文の送り
// サーバ時刻からの経過だけで位置を決める。端末ごとの setInterval のずれが
// 積み上がらないので、全員が同じ瞬間に同じ文字数を見ることになる。
// 早押しで中断しても、再開時に questionStart をずらすので経過は連続する。
// =====================================================================
let typeRafId = null;
function typewriterPos(){
  if (!currentText) return 0;
  const elapsed = getServerTime() - questionStart;
  return Math.max(0, Math.min(currentText.length, Math.floor(elapsed / TEXT.typeSpeed)));
}
function paintTypewriter(){
  const pos = typewriterPos();
  if (pos !== typePos || questionEl.textContent.length !== pos) {
    typePos = pos;
    questionEl.textContent = currentText.slice(0, pos);
  }
  return pos;
}
function typewriterFrame(){
  typeRafId = null;
  if (!typewriterRunning) return;
  const pos = paintTypewriter();
  if (pos < currentText.length) typeRafId = requestAnimationFrame(typewriterFrame);
}
let typewriterRunning = false;
function startTypewriter(){
  stopTypewriter();
  typewriterRunning = true;
  typewriterFrame();
}
function stopTypewriter(){
  typewriterRunning = false;
  if (typeRafId) { try { cancelAnimationFrame(typeRafId); } catch(_) {} typeRafId = null; }
}
function pauseTypewriter(){ stopTypewriter(); }
function resumeTypewriter(){ startTypewriter(); }

// 解答タイマー
// buzz.time を起点にした絶対時刻方式。バックグラウンドで setInterval が
// 間引かれても残り時間がずれず、復帰時もそのまま続きから表示できる。
function startAnswerTimer(buzzTime){
  clearInterval(window._aInt);
  const base = (typeof buzzTime === 'number' && buzzTime) ? buzzTime : getServerTime();
  const deadline = base + TEXT.answerTimeLimit * 1000;
  aTimerEl.style.display='block';
  let lastShown = null;
  const tick = async () => {
    const remain = Math.ceil((deadline - getServerTime()) / 1000);
    const shown = Math.max(0, remain);
    if (shown !== lastShown) {
      lastShown = shown;
      aTimerEl.textContent = TEXT.labels.timeoutLabel + shown + TEXT.labels.secondsSuffix;
    }
    if (remain > 0) return;
    clearInterval(window._aInt);
    if (answered || !answerModeActive) return;
    // 解答時間切れイベント
    iAmWrongLocal = true; // ローカル即時反映
    exitAnswerMode();
    try {
      await push(ref(db,`rooms/${roomId}/events`),{
        nick:myNick,correct:false,guess:'時間切れ',
        answer:sequence[idx].answer,type:'answerTimeout',
        questionIndex:idx,timestamp:getServerTime()
      });
      await set(ref(db,`rooms/${roomId}/wrongAnswers/${myNick}`),true);
    } catch(e) { console.error('answerTimeout failed', e); }
    await releaseBuzzOnDisconnect();
    remove(ref(db,`rooms/${roomId}/buzz`));
  };
  tick();
  window._aInt = setInterval(tick, 250);
}

// 次の問題へボタンのカウントダウン＆自動遷移
function startNextBtnCountdown() {
  clearNextBtnCountdown();
  nextBtnCountdownRemain = 5;
  const isFinal = idx+1>=sequence.length;
  const origLabel = isFinal ? TEXT.labels.finalResult : TEXT.labels.nextQuestion;
  nextBtn.textContent = `${origLabel}（${nextBtnCountdownRemain}）`;
  nextBtn.style.minWidth = '8.5em';
  // ボタン色切り替え
  if (isFinal) {
    nextBtn.classList.add('btn-danger');
    nextBtn.classList.remove('btn-primary');
  } else {
    nextBtn.classList.remove('btn-danger');
    nextBtn.classList.add('btn-primary');
  }
  nextBtnCountdownTimer = setInterval(() => {
    nextBtnCountdownRemain--;
    if (nextBtnCountdownRemain > 0) {
      nextBtn.textContent = `${origLabel}（${nextBtnCountdownRemain}）`;
    } else {
      clearNextBtnCountdown();
      // 自動でボタン押下処理。全端末が同時に撃つとトランザクションが無駄に競合するので
      // 少しばらけさせる（先に成立した1台だけが実際に進める）。
      setTimeout(() => { if (!nextBtn.disabled) nextBtn.click(); }, Math.random() * 400);
    }
  }, 1000);
}

// 早押しボタン処理
// 参加者が自分だけのときは負ける相手がいないので、サーバ確定を待たずに解答欄を出す。
// 複数人のときは従来どおりサーバの先着判定を待つ（「取れたと思ったら取れていない」を避ける）。
buzzBtn.addEventListener('click', async (e) => {
  if (!canBuzz()) return;
  createRipple(e);
  if (window.navigator.vibrate) window.navigator.vibrate(50);

  const solo = activeNicks().length <= 1;
  const pressedAt = getServerTime();
  const remainAtPress = remainingQTime;

  if (solo) {
    // 一人なので先着判定は不要。即座に解答モードへ入る（キーボードもこの操作の文脈で開く）。
    enterBuzzPause({ nick: myNick, remain: remainAtPress });
    enterAnswerMode(pressedAt);
    updateBuzzState();
  } else {
    // iOS: ユーザー操作中に不可視の入力へフォーカスしてキーボードを開いておく
    if (isIOS() && roomModeValue === 'input' && !focusProxyInput) {
      try {
        const proxy = document.createElement('input');
        proxy.type = 'text';
        proxy.autocapitalize = 'none';
        proxy.autocomplete = 'off';
        proxy.spellcheck = false;
        proxy.style.position = 'fixed';
        proxy.style.opacity = '0';
        proxy.style.pointerEvents = 'none';
        proxy.style.zIndex = '-1';
        proxy.style.left = '0';
        proxy.style.bottom = '0';
        proxy.style.width = '1px';
        proxy.style.height = '1px';
        document.body.appendChild(proxy);
        proxy.focus();
        focusProxyInput = proxy;
        setTimeout(() => {
          if (focusProxyInput === proxy) {
            try { proxy.blur(); } catch(_) {}
            if (proxy.parentNode) proxy.parentNode.removeChild(proxy);
            focusProxyInput = null;
          }
        }, 8000);
      } catch(_) {}
    }
    clearInterval(window._qInt);
    pauseTypewriter();
    pausedRemainingQTime = remainAtPress;
    statusEl.textContent = `${myNick} さんが押しました（判定中…）`;
  }

  const buzzRef = ref(db, `rooms/${roomId}/buzz`);
  await runTransaction(buzzRef, current => {
    if (current === null) {
      // remain を載せて、再開時刻を全端末で同じ値から計算できるようにする
      return { nick: myNick, time: pressedAt, remain: remainAtPress };
    }
    return;
  }, { applyLocally: false }).then(async result => {
    if (!result.committed) {
      exitAnswerMode();
      get(buzzRef).then(snap => {
        const buzzData = snap.val();
        const who = buzzData && buzzData.nick ? buzzData.nick : '他のプレイヤー';
        statusEl.textContent = `${who} さんが先に押しました…`;
      }).catch(()=>{});
      answered = false;
      // ここでは再開しない。他参加者同様、サーバからのwrongGuess/answerTimeoutイベントで再開を待つ
    } else {
      // 早押しを確保できた。離脱してもロックが残らないよう自動解放を予約する。
      await claimBuzzOnDisconnect(buzzRef);
      // 複数人のときの解答欄表示は watchBuzz 側（サーバ確定後）で行う
    }
  }).catch(err => {
    console.error('buzz failed', err);
    exitAnswerMode();
    showToast('早押しの送信に失敗しました', null, null, 2500);
  });
});

// リップルエフェクト生成
function createRipple(e) {
  const btn = buzzBtn;
  let ripple = btn.querySelector('.ripple');
  if (ripple) ripple.remove();
  ripple = document.createElement('span');
  ripple.className = 'ripple';
  const rect = btn.getBoundingClientRect();
  const size = Math.max(rect.width, rect.height);
  ripple.style.width = ripple.style.height = size + 'px';
  const x = e.clientX - rect.left - size / 2;
  const y = e.clientY - rect.top - size / 2;
  ripple.style.left = x + 'px';
  ripple.style.top = y + 'px';
  btn.appendChild(ripple);
  setTimeout(() => ripple.remove(), 600);
}

// 解答提出

// 解答ボタンのクリック・Enter対応
async function submitAnswer() {
  answerBtn.disabled = true;
  const rawGuess = answerInput.value;
  const guess = normalizeJa(rawGuess);
  // 空欄でも回答可能にし、不正解として処理
  const rawAnswer = sequence[idx].answer;
  const corr = normalizeJa(rawAnswer);
  const isCorrect = (guess === corr && guess !== '');

  // 入力モード: 手元で即時フィードバック（全員表示はイベント経由でも行われる）
  if (roomModeValue === 'input') {
    showFeedback(isCorrect);
  }

  // イベントオブジェクト作成
  const ev = {
    nick: myNick,
    correct: isCorrect,
    guess: rawGuess, // 表示・履歴用に元の入力を保持
    answer: rawAnswer, // 表示・履歴用に元の正解を保持
    questionIndex: idx,
    timestamp: getServerTime()
  };
  if (!isCorrect) ev.type = 'wrongGuess';
  await push(ref(db, `rooms/${roomId}/events`), ev);

  if (isCorrect) {
    // 正解処理（スコア加算はwatchEventsで行う）
    clearTimers();
    answered = true;
    flowStarted = false;
    pausedRemainingQTime = null;
  revealFullQuestionAndStopSync();
    qTimerEl.textContent = '正解：' + rawAnswer; // 元の正解を表示
    qTimerEl.style.display = 'block';
    buzzBtn.disabled = true;
    exitAnswerMode();
    aTimerEl.style.display = 'none';
    nextBtn.disabled = false;

    await releaseBuzzOnDisconnect();
    await remove(ref(db, `rooms/${roomId}/buzz`));
    clearStaleBuzzWatch();
    updateBuzzState();
    startNextBtnCountdown();
  } else {
    // 誤答処理
  // ローカル即時フラグで一瞬の再有効化を防ぐ
  iAmWrongLocal = true;
  clearTimers();
  await set(ref(db, `rooms/${roomId}/wrongAnswers/${myNick}`), true);
    exitAnswerMode();
    await releaseBuzzOnDisconnect();
    await remove(ref(db, `rooms/${roomId}/buzz`));
    clearStaleBuzzWatch();
    // 再開は watchEvents の wrongGuess で処理
    // 入力モードでは誤答者本人にも体感遅延なく再開させる（イベント往復待ちを避ける）
    if (roomModeValue === 'input') {
      // 全員誤答時はローカル再開をスキップ（正答表示を上書きしない）
      if (answered || !flowStarted) return;
      try {
        // watchEvents の wrongGuess 再開処理を軽量コピー
        // 他端末と同じ値になるよう、buzz の remain とイベント時刻から算出する
        const remain = (typeof lastBuzzRemain === 'number')
          ? lastBuzzRemain
          : (pausedRemainingQTime != null ? pausedRemainingQTime : null);
        if (remain != null) {
          questionStart = computeResumeStart(ev.timestamp, remain);
          set(ref(db, `rooms/${roomId}/qStart/${idx}`), questionStart).catch(()=>{});
        }
        flowStarted = true;
        // タイプ再開（既に全部出ていない場合）
        currentText = sequence[idx].question;
        if (questionEl.textContent.length < currentText.length) {
          typePos = questionEl.textContent.length;
          resumeTypewriter();
        }
        // タイマー再開
  lastDisplayedQSec = null; // 互換のため残すが未使用
        clearInterval(window._qInt);
  lastDisplayedQTenth = null;
  window._qInt = setInterval(tickQ,100);
  tickQ();
        pausedRemainingQTime = null;
        lastBuzzRemain = null;
        updateBuzzState();
      } catch(e){ /* noop */ }
    }
  }
}
// 回答ボタン/Enter キーイベント（入力モード用）※ Enter のバブリングで次問へ飛ばないよう停止
if (answerBtn && answerInput) {
  answerBtn.addEventListener('click', submitAnswer);
  answerInput.addEventListener('keydown', e => {
    if (e.key === 'Enter' && !answerBtn.disabled) {
      e.preventDefault();
      e.stopPropagation();
      submitAnswer();
    }
  });
}
// 次へ
nextBtn.addEventListener('click', async () => {
  if (nextBtn.disabled) return;
  clearNextBtnCountdown();
  clearTimers();
  questionEl.textContent = '';
  questionEl.style.visibility = 'hidden';
  statusEl.textContent = '';
  qTimerEl.style.display = 'none';
  aTimerEl.style.display = 'none';
  questionLabelEl.style.visibility = 'hidden';
  if (choiceArea) {
    choiceArea.classList.add('hidden');
    choiceArea.innerHTML = '';
  }
  if (idx + 1 < sequence.length) {
    const curIdxRef = ref(db, `rooms/${roomId}/currentIndex`);
    // 重要: トランザクション内で外部の idx/resolvedIndex が変化すると
    // 再試行時に expected も変化し、意図せず連続加算が発生し得る。
    // そのため、クリック時点の期待値を固定値としてキャプチャする。
    const fixedExpected = (resolvedIndex == null ? idx : resolvedIndex);
    try {
      const txn = await runTransaction(curIdxRef, current => {
        if (typeof current === 'number' && current === fixedExpected && current + 1 <= 999) {
          return current + 1;
        }
        return; // 他端末が先に進めた等
      }, { applyLocally: false });
      if (txn.committed) {
        await set(ref(db, `rooms/${roomId}/settings/preStart`), getServerTime());
      }
    } catch(_) {}
  } else {
    showResults();
  }
});

// 結果表示
async function showResults(){
  window.scrollTo({top: 0, left: 0, behavior: 'auto'});
  quizAppDiv.classList.add('hidden');
  resultsDiv.classList.remove('hidden');
  allowUnload = true;
  // ルーム終了印（参加・再参加ブロック用）
  try { await set(ref(db,`rooms/${roomId}/settings/finishedAt`), getServerTime()); } catch(_) {}
  clearSession();
  // リスナー解除 & タイマー停止（リソース解放）
  detachWatchers();
  detachPresence();
  clearStaleBuzzWatch();
  releaseBuzzOnDisconnect();
  clearTimers();
  stopHeartbeat();
  // 最新プレイヤー/スコア/イベント/設定を取得（途中参加対策でリアルタイム蓄積に依存しない）
  const [ps, sc, evSnap, settingsSnap] = await Promise.all([
    get(ref(db,`rooms/${roomId}/players`)),
    get(ref(db,`rooms/${roomId}/scores`)),
    get(ref(db,`rooms/${roomId}/events`)),
    get(ref(db,`rooms/${roomId}/settings`))
  ]);
  players = ps.val() || {};
  const eventsObj = evSnap.val() || {};
  const eventsArr = Object.values(eventsObj).filter(Boolean).sort((a,b)=>(a.timestamp||0)-(b.timestamp||0));
  const serverScores = sc.val() || {};

  // 正解数は events から自前で集計する。
  // 各問の勝者は「correct:true の最古のイベント」で、サーバ側 onCorrectEvent と同じ規則。
  // Cloud Function の集計完了を待たずに済むので、最後の1問の加点が抜けることがない。
  const winnerByQ = new Map();
  eventsArr.forEach(e => {
    if (!e.correct) return;
    const qi = Number(e.questionIndex);
    if (!Number.isFinite(qi)) return;
    const cur = winnerByQ.get(qi);
    if (!cur || (e.timestamp || 0) < (cur.timestamp || 0)) winnerByQ.set(qi, e);
  });
  const tally = {};
  winnerByQ.forEach(e => { tally[e.nick] = (tally[e.nick] || 0) + 1; });
  scores = tally;
  // サーバ側と食い違ったら記録だけ残す（集計遅延か不具合かの切り分け用）
  try {
    const keys = new Set([...Object.keys(tally), ...Object.keys(serverScores)]);
    const diff = [...keys].filter(k => (tally[k]||0) !== (serverScores[k]||0));
    if (diff.length) console.warn('score mismatch (local vs server)', diff.map(k => `${k}: ${tally[k]||0}/${serverScores[k]||0}`));
  } catch(_) {}

  const scoreValues = Object.values(tally);
  const maxScore = scoreValues.length ? Math.max(...scoreValues) : 0;
  const winners = maxScore > 0 ? Object.keys(tally).filter(n => tally[n] === maxScore) : [];

  // ゲームログは、rooms/{roomId}/settings/finishedAt が作成されたタイミングで
  // バックエンド (functions/index.js) の onRoomFinished トリガーにより自動的にFirestoreに保存されます。

  // ニックネームはユーザー入力なので、HTML文字列ではなくDOMで組み立てる
  const el = (tag, cls, text) => {
    const n = document.createElement(tag);
    if (cls) n.className = cls;
    if (text != null) n.textContent = text;
    return n;
  };
  resultsDiv.innerHTML = '';
  resultsDiv.appendChild(el('h2', null, TEXT.labels.resultsTitle));
  resultsDiv.appendChild(el('h3', 'champion-announcement',
    winners.length ? `🏆 優勝者：${winners.join('、')}（${maxScore}問正解）` : '🏆 優勝者なし'));

  resultsDiv.appendChild(el('h3', null, TEXT.labels.participantsHeader));
  const ul = el('ul');
  // 終了時に切断していた人が欠けないよう、players と集計結果の和集合で出す
  const participantNicks = Array.from(new Set([
    ...Object.keys(players || {}),
    ...Object.keys(serverScores || {}),
    ...Object.keys(tally)
  ]));
  participantNicks.forEach(nick => {
    const li = el('li', winners.includes(nick) ? 'winner' : null, `${nick}：${tally[nick] || 0}問正解`);
    ul.appendChild(li);
  });
  resultsDiv.appendChild(ul);

  // 章ごとの正答率（自分の成績）
  const byChapter = {};
  sequence.forEach((q, i) => {
    const ch = Number(q && q.chapter);
    if (!Number.isFinite(ch)) return;
    if (!byChapter[ch]) byChapter[ch] = { total: 0, mine: 0 };
    byChapter[ch].total++;
    const w = winnerByQ.get(i);
    if (w && w.nick === myNick) byChapter[ch].mine++;
  });
  const chapterKeys = Object.keys(byChapter).map(Number).sort((a,b)=>a-b);
  if (chapterKeys.length) {
    resultsDiv.appendChild(el('h3', null, 'あなたの章別成績'));
    const cul = el('ul', 'chapter-breakdown');
    chapterKeys.forEach(ch => {
      const c = byChapter[ch];
      cul.appendChild(el('li', null, `${CHAPTER_NAMES[ch] || ch}：${c.mine} / ${c.total}問`));
    });
    resultsDiv.appendChild(cul);
  }

  resultsDiv.appendChild(el('h3', null, TEXT.labels.perQuestionHeader));
  sequence.forEach((q, i) => {
    const card = el('div', 'result-question-card');
    card.appendChild(el('h4', null, `第${i+1}問： ${q.question}`));
    card.appendChild(el('p', null, `正解： ${q.answer}`));
    const qul = el('ul');
    const qEvents = eventsArr.filter(e => Number(e.questionIndex) === i);
    const win = qEvents.filter(e=>e.correct).map(e=>e.nick);
    const winLi = el('li', null, `${TEXT.labels.correctLabel}${win.length ? win.join('、') : 'なし'}`);
    if (win.includes(myNick)) winLi.classList.add('my-result');
    qul.appendChild(winLi);
    qEvents.filter(e=>!e.correct).forEach(e=>{
      let disp='';
      if (e.guess === '' && e.type === 'wrongGuess') disp = '空欄';
      else if (!e.guess || e.guess === '時間切れ' || e.type === 'answerTimeout') disp = '時間切れ';
      else disp = e.guess;
      const li = el('li', null, `${TEXT.labels.incorrectLabel}${e.nick}（${disp}）`);
      if (e.nick === myNick) li.classList.add('my-result');
      qul.appendChild(li);
    });
    card.appendChild(qul);
    resultsDiv.appendChild(card);
  });

  const backBtn = el('button', 'btn-primary', TEXT.labels.returnHome);
  backBtn.id = 'backBtn';
  backBtn.addEventListener('click', () => {
    allowUnload = true;
    window.scrollTo({top: 0, left: 0, behavior: 'auto'});
    location.reload();
  });
  resultsDiv.appendChild(backBtn);
}



// 後片付けは pagehide 側で行う（unload は発火保証がなく bfcache も阻害するため使わない）

// --- 最終結果画面の空欄回答と時間切れの区別 ---
// statusElにテキストをセット。空の場合は全角スペースで高さ維持
function setStatus(text) {
  statusEl.textContent = text && text.trim() ? text : '　';
}

// 日本語の軽い正規化（全角/半角・前後空白・一般記号の除去）
function normalizeJa(s) {
  if (!s) return '';
  let t = (''+s).trim();
  // 全角英数字→半角、全角スペース→半角
  t = t.replace(/[！-～]/g, ch => String.fromCharCode(ch.charCodeAt(0) - 0xFEE0));
  t = t.replace(/　/g, ' ');
  // 句読点・記号など軽めに除去（必要に応じて拡張）
  t = t.replace(/[\s\u3000]+/g, ' ');
  t = t.replace(/[、。・,\.\-_/\(\)\[\]{}\u3001\u3002\u30fb]/g, '');
  // ひらがなをカタカナに変換して比較のブレを吸収
  t = t.replace(/[\u3041-\u3096]/g, ch => String.fromCharCode(ch.charCodeAt(0) + 0x0060));
  // アルファベットを小文字に統一
  return t.toLowerCase();
}
